...
(function () {
      ...
      if (snap.exists()) {
        snap.forEach(function (c) {
          ...
          if (score >= 40) {
            ...
            /* ✅ Bug 19 Fix (updated 2026-08-01): raw Supabase UPDATE replaced with
               admin_set_fraud_score RPC — v32.18 revoked direct fraud_score writes */
            if (window._supa && window._supaAuthed) {
              window._supa.rpc('admin_set_fraud_score', { p_uid: c.key, p_score: score })
                .then(function(res){ if (res.error) console.warn('[fa64] Supabase fraud score sync failed:', res.error.message); })
  ...
  // ✅ FIX (live-testing): window.db/adminDb existing isn't the same as the
  // Supabase bridge being installed — wait for _isSupaBridge before querying
  // `users` (Supabase-only), same class fix as fa66/fa68 above.
  // Auto-run daily
  var key = '_fa64_' + new Date().toISOString().slice(0,10);
  var _t = 0, _iv = setInterval(function () {
    _t++;
    if (window.rtdb && window.rtdb._isSupaBridge && window._supaAuthed) {
      clearInterval(_iv);
        ...
        (window.adminDb || window.db).ref('users').once('value', function (snap) {
          ...
          snap.forEach(function (c) {
            ...
            if (score > 0) {
              ...
              /* FIX Bug#11 (updated 2026-08-01): raw Supabase UPDATE replaced with
                 admin_set_fraud_score RPC — v32.18 revoked direct fraud_score writes */
              if (window._supa && window._supaAuthed) {
                window._supa.rpc('admin_set_fraud_score', { p_uid: c.key, p_score: score })
                  .then(function(res){ if (res.error) console.warn('[Bug#11 Fix] Fraud score Supabase sync:', res.error.message); })
...
(function () {
  ...
  // ✅ FIX (live-testing, same class as fa56-fa62's checker): window.db is a
  // bridge-aware SHIM present from script-load, independent of whether the
  // Supabase bridge has actually installed — so `window.db || window.adminDb`
  // was true almost immediately, well before window.rtdb._isSupaBridge. This
  // function queries `matches` (Supabase-only) and can auto-cancel/refund
  // real matches — the highest-stakes function in this bundle — so it must
  // never fire against an un-bridged Firebase handle. Wait for the bridge
  // explicitly instead.
...
(function () {
  ...
  // ✅ FIX (live-testing): report body queries `matches` (Supabase-only,
  // stale comment above said "Firebase-primary" — no longer true post-
  // migration) via window.adminDb||window.db, which is a shim present from
  // script-load, not proof the Supabase bridge has installed. Wait for the
  // bridge explicitly.
...
(function () {
  ...
  // ✅ FIX (live-testing): same window.db-shim-vs-bridge-readiness bug as
  // fa66/fa56-fa62 — wait for the actual Supabase bridge, not just the shim.
  // Check on load
  var _t = 0, _iv = setInterval(function () {
    _t++;
    if (window.rtdb && window.rtdb._isSupaBridge) { clearInterval(_iv); setTimeout(window.fa68_checkSeasonReset, 10000); }
    if (_t > 60) clearInterval(_iv);
  }, 600);
