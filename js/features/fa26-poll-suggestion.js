...
window.showPollManager = function() {
  ...
  /* ✅ BUG FIX (2026-08-23): "Poll Manager late khulta hai". The button
     click did nothing visible at all until the Supabase query's .then()
     fired — no modal, no spinner, nothing — so a normal network round
     trip (even a fast one) LOOKED like the button was unresponsive or
     broken rather than just loading. Now opens the modal immediately
     with a spinner and fills it in once data arrives, same pattern
     every other well-behaved section in this admin panel already uses. */
  _adminModal('📊 Poll Manager', '<div style="text-align:center;padding:30px"><i class="fas fa-spinner fa-spin" style="font-size:22px;color:#00d4ff"></i></div>');
  ...
  supa.from('polls')
    /* ✅ BUG FIX (2026-08-24): "User ne poll me vote kiya, lekin vote
       admin panel me dikh hi nahi raha". Root cause: this SELECT asked
       for a column called `votes` — but cast_poll_vote (the RPC the
       User Panel's real voting flow calls) writes tallies to a
       DIFFERENT column, `vote_counts`. `votes` exists in the schema but
       is legacy/unused and always empty, so poll.votes was always
       undefined here → votesMap={} → totalVotes=0 → every option shows
       0% (0) no matter how many real votes exist. Confirmed live: the
       DB already had vote_counts:{opt1:1}, total_votes:1 for the exact
       poll in question, sitting there correctly the whole time. Also
       added total_votes itself so we don't even need to re-derive it. */
    .select('id,title,options,vote_counts,total_votes,status,created_at,image_url')
    ...
    .then(function (r) {
      ...
      polls.forEach(function(poll) {
        /* ✅ BUG FIX (2026-08-24) continued: options is stored as a
           KEYED object ({opt1:{label,votes}, opt2:{...}}), and
           vote_counts uses those SAME keys (opt1, opt2, ...) — but
           Object.values(poll.options) throws the keys away, so the old
           optKey fallback (opt.id || label) tried to look up
           vote_counts["Option1"] instead of vote_counts["opt1"] and
           NEVER matched even after fixing the column name above. Now
           preserving the real key explicitly. */
        var opts = Array.isArray(poll.options)
          ? poll.options.map(function(o, i) { return (typeof o === 'string') ? { key: 'opt'+(i+1), label: o } : { key: o.id || o.key || ('opt'+(i+1)), label: o.label || o.text || 'Option' }; })
          : Object.keys(poll.options || {}).map(function(k) { var o = poll.options[k]; return { key: k, label: (o && typeof o === 'object') ? (o.label || k) : (o || k) }; });
        var votesMap = poll.vote_counts || {};
        var totalVotes = Number(poll.total_votes) || Object.values(votesMap).reduce(function(a,b){ return a + (Number(b)||0); }, 0);
        ...
        opts.forEach(function(opt) {
          var votes  = Number(votesMap[opt.key] || 0);
          ...
          h += '<span>' + opt.label + '</span><span style="color:#00d4ff">' + pct + '% (' + votes + ')</span></div>';
      ...
      /* ✅ BUG FIX (2026-08-24): now that `polls` is actually in the
         realtime publication (see migration), auto-refresh this modal
         live when a vote comes in instead of only updating on manual
         reopen. */
      if (window._pollMgrCh) { try { window._pollMgrCh.unsubscribe(); } catch(e){} }
      window._pollMgrCh = supa.channel('poll-mgr-live')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'polls' }, function() {
          if (document.getElementById('adminModalBody') || document.querySelector('.admin-modal')) window.showPollManager();
        })
        .subscribe();
    }).catch(function(e) {
