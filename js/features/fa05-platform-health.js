...
(function() {
  ...
  function runHealthCheck(callback) {
    ...
    if (!rtdb) return;
    /* ✅ FIX (live-testing): guard here too, not just at the auto-run
       interval — the manual "Refresh" button (fA05Health.refresh())
       calls showHealthDashboard() → runHealthCheck() directly, bypassing
       the interval's bridge-ready check entirely. If pressed early
       (e.g. immediately after login, before the bridge finishes
       installing), this would still hit raw Firebase and fail the same
       way. */
    if (!rtdb._isSupaBridge) {
      if (callback) callback({
        openMatches: 0, liveMatches: 0, completedUnpaid: 0, pendingVerif: 0,
        pendingUpdates: 0, pendingWithdrawals: 0, totalUsers: 0, activeUsers: 0,
        alertCount: 0, alerts: [{ type: 'warning', msg: 'Still connecting — try refresh in a few seconds.' }]
      });
      return;
    }
    ...
    rtdb.ref('walletRequests').orderByChild('status').equalTo('pending').once('value', function(s) {
      results.pendingWithdrawals = 0; // always start at 0 and count manually
      if (s.exists()) {
        s.forEach(function(c) {
          var v = c.val();
          // Only count actual withdrawal requests, NOT deposits or diamond purchases
          if (v && (v.type === 'withdraw' || v.type === 'withdrawal') && v.status === 'pending') {
            results.pendingWithdrawals++;
          }
        });
      }
      checkDone();
  ...
  var _check = setInterval(function() {
    _try++;
    /* ✅ FIX (live-testing): previously this only waited for window.db
       or window.rtdb to EXIST — which is true from page-load (raw
       Firebase), well before the Supabase bridge installs. Since
       matches/users/profileRequests/profileUpdates/walletRequests/
       matchFeedback are Supabase-only paths (blocked in Firebase Rules
       by design), running the health check against raw Firebase always
       threw permission_denied — 6 times, every single page load. Wait
       for the bridge itself (window.rtdb._isSupaBridge) instead. */
    if (window.rtdb && window.rtdb._isSupaBridge) {
      clearInterval(_check);
      // Run initial health check after 2s (bridge is already confirmed
      // ready, so no need for the old fixed 5s guess)
      setTimeout(function() {
        runHealthCheck(function(data) {
