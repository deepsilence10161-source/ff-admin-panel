...
(function () {
  ...
  window.showQuickMatchCreate = function () {
    ...
    h += '<div class="form-group"><label>Match Time</label><input type="datetime-local" id="qmTime" class="form-input" onchange="window._onQmMatchTimeChange()"></div>';
  ...
  window._createQuickMatch = async function () {
    console.log('[_createQuickMatch] BUILD 20260907a — double-read settle-check for datetime-local');
    var timeEl = _$('qmTime');
    if (timeEl) {
      timeEl.blur();
      ...
      if (_qmRead1 !== _qmRead2) {
        console.warn('[_createQuickMatch] datetime-local value changed between reads — WebView commit race caught live:', _qmRead1, '→', _qmRead2);
      }
      var _qmSettled = _qmRead2 || _qmRead1;
      if (_qmSettled) _qmCapturedMatchTime = _qmSettled;
    }
