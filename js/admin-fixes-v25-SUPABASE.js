...
(function () {
  ...
  onReady(function() {
    ...
    /* ─────────────────────────────────────────────────────────────
       2. saveTournament — bridge already routes rtdb.ref('matches').push()
          to Supabase INSERT automatically. No wrapper needed here.
          (Removed 2026-08: this used to be a wrapper that called the
          original and did nothing else — confirmed dead code during
          cleanup, not a functional fix.)
    ───────────────────────────────────────────────────────────── */

    /* ─────────────────────────────────────────────────────────────
       3. loadTournaments — bridge-based fetch in admin-inline.js
          already fetches AND renders correctly in one pass. No
          wrapper needed here.
          (Removed 2026-08: this used to be a wrapper that called the
          original and did nothing else — confirmed dead code during
          cleanup, not a functional fix.)
    ...
    /* Update badges immediately, then every 30s as a safety-net only —
       ...
       stale). Every one of these 5 tables is now in the
       supabase_realtime publication (added this session's DB delta), so
       add a live postgres_changes subscription per table and recompute
       the badge the instant a row changes — the 30s poll becomes a rare
       safety net instead of the only update path. */
    updateBadgeCounts();
    setInterval(updateBadgeCounts, 30000);
    window._updateBadgeCounts = updateBadgeCounts;
    try {
      window._supa.channel('v25_badge_counts_live')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'profile_requests' }, updateBadgeCounts)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'profile_updates' }, updateBadgeCounts)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'join_requests' }, updateBadgeCounts)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'disputes' }, updateBadgeCounts)
        .on('postgres_changes', { event: '*', schema: 'public', table: 'team_requests' }, updateBadgeCounts)
        .subscribe();
    } catch(e) {}

    /* ✅ REMOVED (2026-08-19): section 6 ("setupWalletListener via
       Supabase") deleted — window.setupWalletListener no longer exists
       (Wallet Requests tab removed from admin-inline.js), so the
       `if (typeof _origSetupWalletListener === 'function')` guard here
       already made this whole block permanently inert; deleting it
       outright for clarity rather than leaving confusing dead code that
       references a function nobody will ever find. */
            ...
            .then(function(r) {
              ...
              (r.data || []).forEach(function(req) {
                requests[req.id] = {
                  ...
                  requestedFfUid: req.requested_uid || '',
                  ffUid:         req.requested_uid || '',
                  phone:         req.phone || '',
    ...
    function _supaUserToFirebase(u) {
      ...
      return {
        ...
        profileVerified: u.profile_status === 'approved', /* ✅ FIX (2026-08): profile_verified column doesn't exist, derive from profile_status */
    ...
    window._verifyMigration = async function() {
      ...
      for (var i = 0; i < tables.length; i++) {
        var t = tables[i];
        try {
          var r = await supa.from(t).select('id').limit(5000);
          console.log('[v25] Supabase ' + t + ':', (r.data ? r.data.length : 0), 'rows');
...
/* ═══════════════════════════════════════════════════════════════════════════
   FINAL, CONSOLIDATED approveProfile / rejectProfile
   ─────────────────────────────────────────────────────────────────────────
   Root cause of "approve does nothing": window.approveProfile was
   redefined 5 times across admin-inline.js → features-admin.js → v21 →
   v22 → v23. features-admin.js's version uses signature
   (reqKey, uid, ign, ffUid) — 4 args — but every wrapper after it
   (v21/v22/v23) was written assuming the ORIGINAL admin-inline.js
   1-argument signature (rid) and forwarded only that single argument via
   `arguments`. So by the time the chain reached features-admin's version,
   `uid`/`ign`/`ffUid` were always undefined — the users-table update
   silently targeted 'users/undefined' and never actually set the
   player's ign/ff_uid, even though profile_requests.status could still
   flip to 'approved'. This defines ONE final version here, after every
   other script has loaded, so there's no ambiguity about which runs.
   ═══════════════════════════════════════════════════════════════════════ */
(function () {
  var _apprLocks = {};

  window.approveProfile = async function (rid) {
    if (!rid) { if (window.showToast) window.showToast('❌ Invalid request', true); return; }
    if (_apprLocks[rid]) { if (window.showToast) window.showToast('⏳ Already processing…', true); return; }
    _apprLocks[rid] = true;
    try {
      var supa = window._supa;
      /* ✅ FIX (live-testing — confirmed root cause of "Approve/Reject
         profile button doesn't work"): checking only `!supa` isn't
         enough — window._supa exists as an ANON client from page-load,
         well before syncFirebaseToken() authenticates it and sets
         window._supaAuthed. admin_approve_profile calls
         is_caller_admin(), which reads auth.jwt()->>'sub' — on the anon
         client that's null, so is_caller_admin() returns false and the
         RPC returns { success:false, error:'not_authorized' } every
         time, even though the admin genuinely is one. Same root-cause
         class as the other permission_denied bugs fixed earlier this
         session. */
      if (!supa || !window._supaAuthed) { if (window.showToast) window.showToast('❌ Not ready — try again in a moment', true); return; }

      var res = await supa.rpc('admin_approve_profile', { p_request_id: rid });
      if (res.error) { if (window.showToast) window.showToast('❌ Error: ' + res.error.message, true); return; }
      var d = res.data;
      if (!d || !d.success) {
        var msg = { not_authorized: 'Not authorized', request_not_found: 'Request not found',
          already_reviewed: 'Already reviewed', ign_taken: 'IGN already taken',
          ff_uid_taken: 'FF UID already used by another account' }[(d || {}).error] || 'Failed';
        if (window.showToast) window.showToast('❌ ' + msg, true);
        return;
      }

      if (window.showToast) window.showToast('✅ Profile approved' + (d.ign ? ': ' + d.ign : '') + '!');
      if (typeof window.setupProfileListener === 'function') window.setupProfileListener();
    } catch (e) {
      console.error('[approveProfile] error:', e);
      if (window.showToast) window.showToast('❌ Error: ' + e.message, true);
    } finally {
      delete _apprLocks[rid];
    }
  };

  window.rejectProfile = async function (reqKey) {
    if (!reqKey) { if (window.showToast) window.showToast('❌ Invalid request', true); return; }
    try {
      var supa = window._supa;
      /* ✅ FIX (live-testing): same auth-race fix as approveProfile above. */
      if (!supa || !window._supaAuthed) { if (window.showToast) window.showToast('❌ Not ready — try again in a moment', true); return; }
      var res = await supa.rpc('admin_reject_profile', { p_request_id: reqKey });
      if (res.error) { if (window.showToast) window.showToast('❌ Error: ' + res.error.message, true); return; }
      if (!res.data || !res.data.success) { if (window.showToast) window.showToast('❌ ' + ((res.data || {}).error || 'Failed'), true); return; }
      if (window.showToast) window.showToast('Profile rejected');
      if (typeof window.setupProfileListener === 'function') window.setupProfileListener();
    } catch (e) {
      console.error('[rejectProfile] error:', e);
      if (window.showToast) window.showToast('❌ Error: ' + e.message, true);
    }
  };

  console.log('[v25-final] approveProfile/rejectProfile now call admin_approve_profile/admin_reject_profile RPCs (MIGRATION_V33)');
})();
