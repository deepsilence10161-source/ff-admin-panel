 ...
 * ║                                                                  ║
 * ║  RULE (2026-08, Junaid confirmed): Firebase SIRF non-data       ║
 * ║  realtime mechanisms ke liye. Koi bhi actual data Supabase mein  ║
 * ║  hi jaana chahiye — bina exception.                              ║
 * ║                                                                  ║
 * ║  Firebase RTDB SIRF in cheezoin ke liye:                        ║
 * ║    ✅ support/ chats/ supportChats/ supportTyping/ — chat         ║
 * ║    ✅ deviceJoins/ flaggedDevices/ deviceBlacklist/ — anti-cheat  ║
 * ║       fingerprinting (realtime lookup mechanism, data nahi)      ║
 * ║    ✅ .info/ presence/ — Firebase ka apna connection-state        ║
 * ║       system, Supabase mein equivalent hi nahi hai                ║
 * ║    ✅ Firebase Auth — Admin login (email/pass)                   ║
 * ║    ✅ Firebase Analytics + Crashlytics — Auto SDK                ║
 * ║                                                                  ║
 * ║  ✅ MIGRATED TO SUPABASE (2026-08): appSettings/ adminConfig/     ║
 * ║  appConfig/ (→ app_settings table), analytics/ platformEarnings/ ║
 * ║  seasonStats/ (→ platform_earnings, season_stats tables),        ║
 * ║  tdsRecords/ tdsHeld/ (→ tds_records, tds_held tables),          ║
 * ║  adminActions/ (→ admin_actions table), earlyAccessUsers/        ║
 * ║  (→ early_access_users table). Dead-feature paths creatorVideos/ ║
 * ║  videoReports/ and users/$uid/videoStrikes removed entirely —    ║
 * ║  video-sharing feature is fully retired, no migration needed.   ║
 * ║                                                                  ║
 * ║  NOTE: 'admins', 'activityLogs', 'adminAlerts' TABLE_MAP se      ║
 * ║  Supabase pe route hote hain (admins, admin_activity_log,        ║
 * ║  admin_alerts tables).                                          ║
 * ║                                                                  ║
 * ║  Baaki SABB → Supabase                                          ║
 * ╚══════════════════════════════════════════════════════════════════╝
...
(function () {
  ...
  /* ═══════════════════════════════════════════════════════════════════
     1. FIREBASE-ONLY PATHS — sirf non-data realtime mechanisms
  ═══════════════════════════════════════════════════════════════════ */
  var FIREBASE_ONLY = [
    ...
    /* ── Presence / connection (Firebase-native, no Supabase equivalent) ── */
    '.info', 'presence'
  ];

  /* ── ✅ REMOVED (2026-08): 'users/{uid}/videoStrikes' Firebase-only
     exception hata diya gaya — Creator Video Review feature hi dead code
     hai ab (video-sharing removed), isliye is field ki migration ki
     zarurat nahi, sirf iska UI/code cleanup ho raha hai. ── */
  ...
  var TABLE_MAP = {
    ...
    /* ✅ FIX (2026-08-21): removed from here — adminNotes/{uid} needs a
       dedicated nested handler (below, in getNestedTableHandler) because
       the row is keyed by user_id, not the table's own `id` (uuid) PK.
       The old generic TABLE_MAP entry mapped p.id straight into the `id`
       column (wrong PK entirely) and treated the note text as if setting
       a whole-row upsert, so rtdb.ref('adminNotes/'+uid).set(text) either
       silently failed or crashed depending on the value — this feature
       never actually saved a note. */
    'platformStats':         { table: 'platform_stats',      id: 'id'      },

    /* ── Additional paths ── */
    'withdrawalRequests':    { table: 'sd_requests',          id: 'id'      },
    /* ✅ FIX (2026-08-19, CRITICAL): this pointed at premium_requests —
       a real, dedicated `season_pass_requests` table exists (confirmed
       live) and was sitting completely empty because every Season Pass
       request/approval was actually being written into premium_requests
       instead, mixing two unrelated features' data in one table (and
       colliding with premium_requests' own row shape — tier/plan_type/
       bundle_id vs season/price/screenshot — none of which line up).
       Season Pass approvals also would have hit the exact same
       "date/time field value out of range" bug as premium approvals did
       (approvedAt: Date.now() with no dedicated converter), just never
       reported since this whole feature had zero real data flowing
       through it to notice. */
    'seasonPassRequests':    { table: 'season_pass_requests', id: 'id'      },
    ...
    /* ── ✅ MIGRATED FROM FIREBASE (2026-08) ── */
    'tdsRecords':            { table: 'tds_records',          id: 'id'      },
    'tdsHeld':               { table: 'tds_held',             id: 'id'      },
    'adminActions':          { table: 'admin_actions',        id: 'id'      },
    'earlyAccessUsers':      { table: 'early_access_users',   id: 'user_id' },
    'platformEarnings':      { table: 'platform_earnings',    id: 'id'      },
  ...
  var TABLES_WITHOUT_CREATED_AT = { 'creator_stats': 1, 'app_settings': 1, 'early_access_users': 1 };
  ...
  function userToSupa(d) {
    ...
    /* ✅ FIX (2026-08): 'profile_verified' write removed — no such column
       exists in the users table (only 'profile_status' does). Every write
       to it was silently failing. profile_status is the single source of
       truth for verification state now; see the userToSupa read-side note
    ...
    /* ✅ FIX (live-testing): approveProfile() also sets profileRequestCount
       (real column, was silently dropped — now mapped) and pendingProfile/
       pendingUid (NEITHER has a corresponding users-table column — verified
       against the live schema; must stay unmapped/dropped here rather than
       guessing a column name, since sending an unknown column to PostgREST
       returns a 400 and silently aborts the whole update from the caller's
       point of view, which is what made Approve/Reject look like it did
       nothing). */
    if (d.profileRequestCount !== undefined) s.profile_request_count = d.profileRequestCount;
  ...
  function userFromSupa(row) {
    ...
    return {
      ...
      /* ✅ FIX (2026-08): 'profile_verified' is not a real column in the
         users table — only 'profile_status' is. Reading it always
         returned false regardless of actual verification state, which
         silently broke every caller checking .profileVerified (verified
         badge always showed "Unverified", and the already-verified
         identity-change safeguard never triggered). Now derived from
         the real column so it actually reflects verification state. */
      profileVerified:   row.profile_status === 'approved',
  ...
  function profileReqToSupa(d) {
    ...
    if (uid) s.user_id = uid;
    /* ✅ FIX: only touch a column when this specific update actually
       carries that field. The old unconditional version defaulted every
       field to '' when absent — so a status-only update (e.g. approve/
       reject sending {status, processedAt, processedBy}) silently wiped
       requested_ign/requested_uid/phone back to blank on every approval. */
    if (d.requestedIgn !== undefined || d.ign !== undefined || d.username !== undefined || d.gameName !== undefined)
      s.requested_ign = d.requestedIgn || d.ign || d.username || d.gameName || '';
    if (d.requestedUid !== undefined || d.requestedFfUid !== undefined || d.ffUid !== undefined || d.gameUid !== undefined)
      s.requested_uid = d.requestedUid || d.requestedFfUid || d.ffUid || d.gameUid || '';
    if (d.phone !== undefined || d.mobileNumber !== undefined)
      s.phone = d.phone || d.mobileNumber || '';
    /* screenshot_url: no such column on profile_requests (verified against
       schema — only processed_at/processed_by were added later via ALTER
       for this table). Writing it would fail the whole update silently
       (supaUpdate doesn't surface PostgREST errors), so it's dropped here. */
    if (d.status !== undefined) s.status = d.status || 'pending';
  ...
  function profileReqFromSupa(row) {
    ...
    return {
      ...
      requestedFfUid: row.requested_uid || '',
      ffUid:          row.requested_uid || '',
      requestedUid:   row.requested_uid || '',
      phone:          row.phone || '',
      status:         row.status           || 'pending',
  ...
  function profileUpdFromSupa(row) {
    ...
    return {
      ...
      processedAt:    row.processed_at ? new Date(row.processed_at).getTime() : null,
      createdAt:      row.created_at ? new Date(row.created_at).getTime() : null,
      /* ✅ FIX (2026-08-21): FF UID change (premium/paid) fields were
         missing from this converter's whitelist, so even though the User
         Panel wrote them fine to Supabase, the Admin Panel's rtdb.ref()
         read path silently dropped them — admin would see a normal
         profile update with no indication it was a paid/premium UID
         change or any payment proof to check. */
      uidChangeRequested: !!row.uid_change_requested,
      uidChangeVia:       row.uid_change_via || '',
  ...
  function profileUpdToSupa(d) {
    ...
    if (d.processedAt)  s.processed_at  = new Date(d.processedAt).toISOString();
    if (d.processedBy)  s.processed_by  = d.processedBy;
    /* ✅ FIX (2026-08-21): same gap as profileUpdFromSupa above, on the
       write side — needed so Admin Panel code (e.g. approveProfileUpdate)
       can also read these back correctly and so nothing silently gets
       dropped if the admin side ever writes one of these fields. */
    if (d.uidChangeRequested !== undefined) s.uid_change_requested = !!d.uidChangeRequested;
    if (d.uidChangeVia)      s.uid_change_via     = d.uidChangeVia;
    if (d.paymentScreenshot) s.payment_screenshot = d.paymentScreenshot;
    if (d.paymentUtr)        s.payment_utr        = d.paymentUtr;
    if (d.paymentAmount)     s.payment_amount     = d.paymentAmount;
    return s;
  }
  ...
  /* ✅ FIX (2026-08-19, CRITICAL): premium_requests previously had no
     dedicated `to` converter — it fell back to genericToSupa(), which
     only does a snake_case key rename (approvedAt → approved_at) and
     passes the VALUE through completely unchanged. approvePremiumReq()
     writes `approvedAt: Date.now()` (a raw millisecond-epoch integer
     like 1787039514717) — genericToSupa let that raw number reach
     PostgREST as the value for approved_at, a real `timestamptz`
     column. Postgres/PostgREST can't parse a bare large integer as a
     timestamp and threw exactly "date/time field value out of range:
     1787039514717" on every single premium approval — matching the
     reported error precisely. Same bug would have hit rejectedAt on
     reject. Fixed by giving this table its own converter that properly
     ISO-stringifies every date-like field, mirroring the pattern
     already used for profile_updates/team_requests elsewhere in this
     bridge. */
  function premiumReqToSupa(d) {
    ...
    if (d.approvedAt)  s.approved_at = new Date(d.approvedAt).toISOString();
    if (d.rejectedAt)  s.rejected_at = new Date(d.rejectedAt).toISOString();
    if (d.reviewedAt)  s.reviewed_at = new Date(d.reviewedAt).toISOString();
    return s;
  }

  /* ── SEASON PASS REQUEST conversions ──
     ✅ ADDED (2026-08-19) alongside the TABLE_MAP fix above — this table
     never had its own converter before since nothing correctly pointed
     at it. season_pass_requests has NO approved_at/rejected_at columns
     (only status + reviewed_by), unlike premium_requests, so approvedAt/
     rejectedAt are intentionally NOT written here — there's no column
     for them yet; only status + reviewed_by (whoever actioned it) are
     persisted. */
  function seasonPassReqFromSupa(row) {
    if (!row) return null;
    return {
      id:               row.id,
      uid:              row.user_id,
      ...
      price:            row.price || 49,
      screenshotBase64: row.payment_screenshot || '',
      status:           row.status || 'pending',
      reviewedBy:       row.reviewed_by || null,
      createdAt:        row.created_at ? new Date(row.created_at).getTime() : null
    };
  }
  function seasonPassReqToSupa(d) {
    if (!d) return {};
    var s = {};
    if (d.status)      s.status      = d.status;
  ...
  /* ── WALLET TRANSACTION (users/{uid}/transactions) conversions ──
     ✅ BUG FIX (2026-08-22): wallet_transactions had NO registered
     converter at all — every push here fell through to genericToSupa,
     which only snake_cases keys ('timestamp' stays 'timestamp', 'type'
     stays 'type'). Neither is a real column (real ones are created_at
     and txn_type), so EVERY admin-side transaction-log write through
     users/{uid}/transactions — refunds, corrections, sky/green diamond
     credits, winnings — failed outright with "Could not find the
     'timestamp' column" (confirmed live, Sky Diamond approval flow).
     The balance credit itself usually happened via a separate RPC call,
     so the money moved but the log entry silently never did — this
     converter fixes the log write itself. */
  function walletTxnToSupa(d) {
    if (!d) return {};
    var s = {};
    if (d.type !== undefined) s.txn_type = d.type;
    if (d.txnType !== undefined) s.txn_type = d.txnType;
    if (d.amount !== undefined) s.amount = Math.abs(Number(d.amount) || 0);
    if (d.currency !== undefined) {
      /* Normalize shorthand currency values used throughout the admin
         panel ('sky', 'green') to the real column values. */
      var cmap = { sky: 'sky_diamonds', green: 'green_diamonds', coin: 'coins', coins: 'coins', sky_diamonds: 'sky_diamonds', green_diamonds: 'green_diamonds' };
      s.currency = cmap[d.currency] || d.currency;
    }
    if (d.description !== undefined) s.description = d.description;
    if (d.reason !== undefined) s.reason = d.reason;
    if (d.note !== undefined) s.note = d.note;
    if (d.refId !== undefined) s.ref_id = d.refId;
    if (d.matchId !== undefined && s.ref_id === undefined) s.ref_id = d.matchId;
    if (d.status !== undefined) s.status = d.status;
    if (d.timestamp !== undefined) s.created_at = new Date(d.timestamp).toISOString();
    if (d.createdAt !== undefined) s.created_at = new Date(d.createdAt).toISOString();
    return s;
  }
  function walletTxnFromSupa(row) {
    if (!row) return null;
    return {
      id: row.id, uid: row.user_id, userId: row.user_id,
      type: row.txn_type, txnType: row.txn_type,
      amount: row.amount, currency: row.currency,
      description: row.description || row.reason || '',
      reason: row.reason, note: row.note, refId: row.ref_id,
      status: row.status,
  ...
  /* ── SPONSORED TOURNAMENT conversions ──
     ✅ BUG FIX (2026-08-23, CRITICAL): "Sponsor match me Create
     Tournament pe click karne par kuch hota hi nahi". sponsored_
     tournaments had NO dedicated converter — every create fell through
     to genericToSupa (plain top-level snake_case rename, nested objects
     passed through unchanged). The client sends {name, sponsor,
     prizePool, prizes:{first,second,third,fourthToTenth}, matchId,
     description, status, prizeDistributed, createdAt} but the real
     columns are title (NOT NULL), sponsor_name, prize_pool — 'name'
     never became 'title' under simple snake-casing, so EVERY insert
     failed outright on the NOT NULL violation, before the newly-added
     prizes/match_id/description/prize_distributed columns even mattered. */
  function sponsoredTournamentToSupa(d) {
    if (!d) return {};
    var s = {};
    if (d.name !== undefined) s.title = d.name;
    if (d.title !== undefined) s.title = d.title;
    ...
    if (d.description !== undefined) s.description = d.description;
    if (d.status !== undefined) s.status = d.status;
    if (d.prizeDistributed !== undefined) s.prize_distributed = d.prizeDistributed;
    if (d.withdrawalUid !== undefined) s.withdrawal_uid = d.withdrawalUid;
    if (d.wdStatus !== undefined) s.wd_status = d.wdStatus;
    if (d.wdAmount !== undefined) s.wd_amount = d.wdAmount;
    if (d.wdUpi !== undefined) s.wd_upi = d.wdUpi;
    return s;
  }
  function sponsoredTournamentFromSupa(row) {
    if (!row) return null;
    return {
      id: row.id,
      name: row.title || '',
      ...
      status: row.status || 'active',
      prizeDistributed: !!row.prize_distributed,
      withdrawalUid: row.withdrawal_uid || null,
      wdStatus: row.wd_status || null,
      wdAmount: row.wd_amount || 0,
      wdUpi: row.wd_upi || '',
      createdAt: row.created_at ? new Date(row.created_at).getTime() : null
    };
  }
  ...
  var CONVERTERS = {
    ...
    'wallet_transactions':   { to: walletTxnToSupa,    from: walletTxnFromSupa  },
    ...
    'sponsored_tournaments': { to: sponsoredTournamentToSupa, from: sponsoredTournamentFromSupa },
    ...
    /* ✅ MIGRATED FROM FIREBASE (2026-08): strip the 'uid' field since
       p.id (the path segment) already maps to user_id via mapping.id —
       'uid' itself isn't a real column and would error the upsert. */
    'early_access_users':    { to: function(d) {
  ...
  function getNestedTableHandler(p) {
    ...
    if (p.root === 'mentors') {
      /* ✅ FIX: mentor_profiles has no 'active' column — the real column
         is 'is_available' (boolean). Also: query results' snap.key is
         always the row's own 'id' (UUID) per this bridge's convention, and
         the only caller (_revokeMentor) passes that key straight through —
         so the id-based filter must match on 'id', not 'user_id'. */
      if (p.field === 'active') return { table: 'mentor_profiles', field: 'is_available', filter: { col: 'id', val: p.id } };
      if (p.id) return { table: 'mentor_profiles', filter: { col: 'id', val: p.id } };
    }
    if (p.root === 'clans') {
  ...
  async function supaRead(p, query) {
    ...
    /* ✅ FIX (2026-08-19, CRITICAL): ffUIDIndex/{ffUid}.once('value') is a
       single-scalar lookup in real Firebase RTDB semantics — the value AT
       that path is just the owning user's uid (a string), exactly what
       the earlier .set(uid) fix (below, in supaSet) writes. But this path
       has no nested .field (getNestedTableHandler returns {table,filter}
       only) and only 2 path parts, so it fell through to the generic
       "top-level table, single row" branch, which does
       `.select('*').eq('ff_uid', p.id).single()` and returns the FULL
       ROW OBJECT ({ff_uid, user_id, created_at}) as .val() — not the
       scalar uid string the callers expect. Both approveProfile (STEP 7)
       and approveProfileUpdate compare `fi.val() !== uid` (object !==
       string is ALWAYS true), so this ALWAYS reported "FF UID already
       taken by another user" even when it was the SAME user's own
       already-registered FF UID — which is the common case (changing
       only IGN, not FF UID). Confirmed live: the only ff_uid_index row
       present pointed at its own correct owner, yet every pending
       profile-update request for that same user (unchanged FF UID) was
       being blocked by this false positive. Special-cased here to return
       the scalar user_id, matching real RTDB behavior. */
    if (p.root === 'ffUIDIndex' && p.id) {
      var fiRes = await supa.from('ff_uid_index').select('user_id').eq('ff_uid', p.id).maybeSingle();
      return (fiRes.data && fiRes.data.user_id) || null;
    }

    /* ✅ FIX (2026-08-21): adminNotes/{uid}.set(text) / .remove() — the
       User Note feature (Admin Panel A33). Row is keyed by user_id, and
       the value is a plain string, not an object — so this needs the
       same field-scalar treatment as battlePass above rather than a
       whole-row upsert against the wrong PK. */
    if (p.root === 'adminNotes' && p.id) {
      return { table: 'admin_notes', field: 'note', filter: { col: 'user_id', val: p.id }, insertPatch: { user_id: p.id } };
    }
    ...
    var r4 = await q3;
    if (r4.error) {
      console.error('[Bridge] supaRead FAILED for table "'+table+'":', r4.error.message, '| code:', r4.error.code, '| details:', r4.error.details);
      return [];
    }
    return r4.data || [];
  }
  ...
  async function supaUpdate(p, data) {
    ...
    /* ✅ FIX (live-testing): PostgREST errors (e.g. unknown column — see
       the pendingProfile bug this session) were silently swallowed here.
       await on a PostgrestFilterBuilder resolves to {data,error,...}
       rather than throwing, so a bad request just returned normally and
       every caller's try/catch saw a clean success. Surface the error so
       callers like approveProfile() actually show it via showToast
       instead of silently no-op'ing. */
    var _upRes = await upQ;
    if (_upRes && _upRes.error) {
      console.error('[Bridge] UPDATE failed on', table, '(path:', p.raw + '):', _upRes.error.message);
      throw new Error('Supabase update failed on ' + table + ': ' + _upRes.error.message);
    }
  }
  ...
  async function supaSet(p, data) {
    ...
    /* ✅ FIX (2026-08-17, CRITICAL): ffUIDIndex/{ffUid}.set(uid) sends a raw
       scalar string (the user's uid), not an object — but getNestedTableHandler
       returns {table, filter} with no .field and no .insertPatch for this path,
       so this used to fall all the way through to the generic "upsert row at
       root table" branch below. That branch calls genericToSupa(data), which
       returns non-object data AS-IS (the raw uid string) since it can't be
       spread into snake_case keys — then tries to do
       upsertData[mapping.id] = p.id, i.e. assigning a property onto a string
       primitive, which throws "Cannot create property 'ff_uid' on string
       '<uid>'" every single time a profile update changed the FF UID.
       Handle this specific scalar-set case explicitly: upsert
       {ff_uid: <newFfUid>, user_id: <uid>} into ff_uid_index. */
    if (p.root === 'ffUIDIndex' && p.id && typeof data !== 'object') {
      await supa.from('ff_uid_index').upsert(
        { ff_uid: p.id, user_id: data },
        { onConflict: 'ff_uid' }
      );
      return;
    }
    ...
    /* ✅ FIX (2026-08): supaSet's upsert path was stamping created_at
       unconditionally, unlike supaInsert which already respects
       TABLES_WITHOUT_CREATED_AT — tables that lack that column (e.g.
       early_access_users, whose timestamp column is joined_at) would
       fail the upsert with "column does not exist". Now this path
       checks the same guard. */
    var setTimeCol = defaultTimeCol(table);
    if (!upsertData[setTimeCol]) upsertData[setTimeCol] = new Date().toISOString();
    if (setTimeCol !== 'updated_at') upsertData.updated_at = upsertData.updated_at || new Date().toISOString();
  ...
  function setupRealtimeListener(p, callback, query) {
    ...
    /* ✅ SPEED FIX (2026-08-24): a burst of rapid changes (e.g. admin
       bulk-updating several matches, or several users joining a match
       within the same second) used to trigger a full doRead() — a
       complete table re-fetch — once PER row change, back-to-back. That
       made bursts feel slower, not faster (multiple overlapping network
       round-trips racing each other for the same render). Coalescing
       into one fetch per 60ms burst window: still feels instant to a
       human, but collapses N rapid change-events into 1 read instead of
       N reads. */
    var _doReadTimer = null;
    var _doReadDebounced = function() {
      clearTimeout(_doReadTimer);
  ...
  /* READ ONCE */
  /* BUG FIX (2026-08): Admin Panel's user-detail "eye" modal showed
     "Unknown"/"N/A" for name/UID and literal broken template text for
     Sky Diamond/Coins etc. Root cause traced here: for a single-record
     read like users/<uid>, supaRead() returns one plain row object
     (not an array). This function used to wrap that single object in
     a 1-element array before calling makeSnapshot() — which flips
     makeSnapshot's val() into its "multiple rows → {id: row}" branch,
     so snap.val() returned { "<uid>": {ign:..., ffUid:...} } instead
     of { ign:..., ffUid:... } directly. Every u.ign / u.ffUid /
     u.skyDiamonds read in the calling code was then undefined, which
     is also what let the pre-existing quote-escaping typo in
     admin-inline.js's openUserModal() surface as visible broken text
     (it was already falling through to the `||` fallback on every
     field, just via a separate bug — fixed separately in that file).
     Fix: pass the single object straight to makeSnapshot() without
     the array wrapper, so val()'s single-row branch (conv.from(rows)
  ...
  function installBridge() {
    ...
    /* ✅ FIX (live-testing, defense-in-depth — 2026-08): live testing kept
       showing permission_denied on matches/profileRequests/walletRequests/
       etc even after every known early-caller (initAdminDashboard,
       watchPendingBadges, fa15/fa17/fa19, tournament auto-refresh, etc)
       was individually guarded to wait for window.rtdb._isSupaBridge.
       Root cause: `var rtdb = _adminApp.database()` in admin-inline.js and
       `window.rtdb = bridge` here are the SAME global binding, so a
       .ref('matches') call issued after the guard passes correctly picks
       up the bridge — BUT any code that resolved `.ref()`'s RETURN VALUE
       into its own variable *before* the bridge installed (e.g.
       `var x = rtdb.ref('matches'); x.on(...)` at some later point) is
       holding a real-Firebase Reference object directly, which no amount
       of re-checking window.rtdb afterward can fix, since that object
       doesn't care what window.rtdb points to now. Realistically this
       shouldn't happen given the file load order, but with ~50 feature
       files this large, one more instance of the same class of bug
       (found across 6+ separate files this session alone) is more likely
       than not. Belt-and-suspenders: patch realRtdb.ref() itself so that
       EVEN a raw, un-bridged Firebase handle routes Supabase-only paths
       to Supabase. This makes the bug structurally impossible to
       reintroduce, instead of relying on every call-site remembering to
       check _isSupaBridge first. */
    var _origRealRef = realRtdb.ref.bind(realRtdb);
    realRtdb.ref = function (path) {
      if (!isFirebasePath(path || '/')) {
        return new SupaRef(path || '/');
      }
      return _origRealRef(path);
    };
  ...
  function _patchGlobalNotifications() {
    window._adminNotifyAll = function(title, body, type) {
      var supa = window._supa;
      if (!supa) return Promise.resolve({success:false, error:'Supabase not ready'});
      /* ✅ FIX (2026-08-21): the RPC returns {success:false, error:'Admin
         only'} as a normal 200 response (it's a jsonb payload, not an
         HTTP/PostgREST error) whenever the caller isn't recognized as
         admin — so .catch() never fired for that case and the button
         looked like it worked with no feedback at all. Now actually
         inspects the returned payload and surfaces a real toast either
         way, instead of silently doing nothing. */
      return supa.rpc('admin_send_broadcast_notification', {
        p_type: type || 'admin_alert', p_title: title, p_body: body
      }).then(function(res) {
        var payload = res && res.data;
        if (res && res.error) {
          console.warn('[Bridge] Global notif RPC error:', res.error.message);
          if (window.showToast) showToast('Broadcast failed: ' + res.error.message, true);
          return {success:false, error:res.error.message};
        }
        if (payload && payload.success === false) {
          console.warn('[Bridge] Global notif rejected:', payload.error);
          if (window.showToast) showToast('Broadcast failed: ' + (payload.error||'unknown'), true);
          return payload;
        }
        return payload || {success:true};
      }, function(e) {
        console.warn('[Bridge] Global notif Supabase fail:', e.message);
        if (window.showToast) showToast('Broadcast failed: ' + e.message, true);
