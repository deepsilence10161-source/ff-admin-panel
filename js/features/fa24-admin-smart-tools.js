...
window.showPrizeCalc = function() {
  ...
  h += '<div><label style="font-size:11px;color:#aaa;display:block;margin-bottom:4px">Entry Fee (₹)</label><input type="number" id="pcFee" class="form-input" placeholder="e.g. 20" oninput="window._calcPrizePreview()" min="0"></div>';
  h += '<div><label style="font-size:11px;color:#aaa;display:block;margin-bottom:4px">Max Players</label><input type="number" id="pcPlayers" class="form-input" placeholder="e.g. 12" oninput="window._calcPrizePreview()" min="1"></div>';
  h += '</div>';
  h += '<div style="margin-bottom:8px"><label style="font-size:11px;color:#aaa;display:block;margin-bottom:4px">Platform Cut %</label><input type="number" id="pcCut" class="form-input" value="15" oninput="window._calcPrizePreview()" min="0" max="50"></div>';
...
window._calcPrizePreview = function() {
...
window._saveNote = async function(uid) {
  ...
  /* ✅ FIX (2026-08-21): the whole User Note feature never actually saved
     anything — adminNotes/{uid} wasn't correctly mapped in the Supabase
     bridge (see supabase-rtdb-bridge.js TABLE_MAP/getNestedTableHandler
     fix). Also explicitly stamp created_by now that the note is actually
     persisted, so admin activity is attributable. */
  await rt.ref('adminNotes/'+uid).set(txt||null);
  if (txt && window._supa) {
    window._supa.from('admin_notes').update({ created_by: (window._adminUid ? window._adminUid() : null) }).eq('user_id', uid).then(null, function(){});
  }
...
window._doCoinManager = async function() {
  ...
  if(!window._supa){if(window.showToast)showToast('Supabase not connected',true);return;}
  /* ✅ BUG FIX (2026-07-17): was a plain Firebase read-then-write against
     the RTDB-bridge path users/{uid}/coins — non-atomic (a concurrent
     coin change between the read and write would be silently lost), and
     that bridge path can no longer write coins directly at all after this
     session's grant lockdown on the coins/sky_diamonds/green_diamonds
     columns (see COMPLETE_SCHEMA.sql's users GRANT block for why — it was
     a severe balance-tampering hole affecting every user, not just this
     admin tool). admin_set_coins is the correct, atomic, admin-checked
     replacement — it also now logs to wallet_transactions, which the old
     version never did. */
  var res = await window._supa.rpc('admin_set_coins', { p_uid: uid, p_action: action, p_amount: amt });
  if (res.error || (res.data && res.data.success === false)) {
    var msg = (res.data && res.data.error) || (res.error && res.error.message) || 'Unknown error';
    if(window.showToast)showToast('❌ Coin update failed: ' + msg, true);
    return;
  }
  var oldBal = res.data.old_balance, newBal = res.data.new_balance;
  var rt=rtdb();
  if (rt) rt.ref('users/'+uid+'/notifications').push({title:'🪙 Coins Updated',message:'Coins '+(action==='add'?'added: +':action==='remove'?'removed: -':'set to: ')+amt+'. New balance: '+newBal,timestamp:Date.now(),read:false}).catch(function(){});
  if(window.showToast)showToast('✅ Coins updated! '+oldBal+' → '+newBal);
  if(window.closeGenericModal)closeGenericModal();
};
