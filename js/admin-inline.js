...
async function saveTournament(){
  console.log('[saveTournament] BUILD 20260907b — scoped join_requests query + full timing instrumentation');
  var _saveWallClockStart = Date.now();
  var saveBtn=document.getElementById('saveTournamentBtn');
  setLoading(saveBtn,true);

  /* ✅ HARDENED (2026-09-07), after the time-mismatch bug kept
     recurring despite multiple fixes: live DB data confirmed the bug
     is INTERMITTENT (some matches save with the correct time, others
     don't, from the exact same code) — the signature of a genuine
     Android WebView timing race in the native datetime-local input
     itself, not a deterministic logic bug in this file. A single
     synchronous .value read right after the admin closes the picker
     can occasionally return a value the WebView hasn't fully committed
     yet, even with no custom code in between.
     Fix: blur() the field, wait one animation frame, then read .value
     TWICE with a short gap and compare. If the two reads agree, the
     value has settled and is trustworthy. If they disagree (the
     in-flight-commit case), take the SECOND read — by definition the
     more-settled one — and log a warning so this is visible in the
     console if it ever happens again instead of silently mis-saving. */
  ...
  if (tMatchTimeEl) {
    ...
    if (_read1 !== _read2) {
      console.warn('[saveTournament] datetime-local value changed between reads — WebView commit race caught live:', _read1, '→', _read2, '— using the settled second read');
    }
    mts = _read2 || _read1;
    if (mts) _capturedMatchTime = mts;
  }
