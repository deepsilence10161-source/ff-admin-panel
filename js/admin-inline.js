...
var _adminApp = firebase.initializeApp(firebaseConfig, "adminPanel");
var auth=_adminApp.auth(),rtdb=_adminApp.database(),db=_adminApp.firestore();
...
auth.onAuthStateChanged(async function(u){
  ...
  if(u){
    try{
      /* Check Firestore admins collection first, then RTDB as fallback */
      var isAdmin=false;
      try{
        var fsSnap=await db.collection('admins').doc(u.uid).get();
        if(fsSnap.exists)isAdmin=true;
      }catch(fsErr){console.log('Firestore check failed, trying RTDB:',fsErr.message);}
      if(!isAdmin){
        try{
          var rtSnap=await rtdb.ref('admins/'+u.uid).once('value');
          if(rtSnap.exists())isAdmin=true;
        }catch(rtErr){console.log('RTDB check failed:',rtErr.message);}
      }
      /* Last resort: check by email for known admin emails */
...
function renderProfileRequests(snap){
  ...
  rows.forEach(function(r){
    ...
    tb.innerHTML+='<tr>'+
      /* Col 1: User Name */
      '<td style="white-space:nowrap"><div style="font-size:13px;font-weight:800">'+displayName+'</div><div class="font-mono text-xxs" style="color:var(--info);cursor:pointer;margin-top:2px" onclick="navigator.clipboard&&navigator.clipboard.writeText(\''+uid+'\').then(function(){showToast(\'UID copied!\')})">'+uid.substring(0,14)+'... <i class="fas fa-copy" style="font-size:8px"></i></div><div class="font-mono text-xxs" style="color:var(--primary);margin-top:1px">FF: '+(currentUser.ffUid||d.ffUid||'—')+'</div></td>'+
...
function renderUsers(){
  ...
  usersSnapshot.forEach(function(ch){
    ...
    var row='<tr>';
    row+='<td style="min-width:110px"><span style="font-weight:700;color:var(--primary)">'+(ign)+'</span><div onclick="navigator.clipboard.writeText(\''+uid+'\').then(function(){showToast(\'Firebase UID copied!\')})" style="font-size:9px;color:var(--text-muted);font-family:monospace;cursor:pointer" title="Click to copy full UID">'+uid.substring(0,10)+'…📋</div></td>';
    row+='<td><span style="font-family:monospace;font-size:11px;color:var(--info);background:rgba(0,212,255,.08);padding:2px 6px;border-radius:5px">'+ff+'</span></td>';
    row+='<td style="font-size:11px;color:var(--text-dim)">'+ph+'</td>';
    row+='<td style="font-size:10px;color:var(--text-muted);max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+em+'</td>';
...
async function processManualWallet(){var uid=document.getElementById('manualUid').value.trim(),act=document.getElementById('manualAction').value,wt=document.getElementById('manualWalletType').value,amt=Number(document.getElementById('manualAmount').value)||0,rsn=document.getElementById('manualReason').value.trim()||'Admin adjustment';if(!uid)return showToast('Enter UID',true);if(amt<=0)return showToast('Amount > 0',true);var s=await rtdb.ref(DB_USERS+'/'+uid).once('value');if(!s.exists())return showToast('Not found',true);
  ...
  try{
    ...
    var currSymbol = wt==='sky' ? '💎' : wt==='green' ? '<img src="green-diamond.png" style="width:14px;height:14px;vertical-align:middle;object-fit:contain;display:inline-block">' : '🪙';
    if(act==='credit') await rtdb.ref(DB_USERS+'/'+uid+'/'+currPath).transaction(function(v){return(v||0)+amt});
    else await rtdb.ref(DB_USERS+'/'+uid+'/'+currPath).transaction(function(v){return Math.max((v||0)-amt,0)});
    await rtdb.ref(DB_USERS+'/'+uid+'/transactions').push({type:act==='credit'?'admin_credit':'admin_debit',currency:wt,amount:act==='credit'?amt:-amt,description:rsn||'Admin manual adjustment',timestamp:Date.now()});
    await rtdb.ref(DB_USERS+'/'+uid+'/notifications').push({title:act==='credit'?'💰 Wallet Credited!':'Wallet Adjusted',message:currSymbol+amt+' '+(act==='credit'?'add kiye gaye':'remove kiye gaye')+'. Reason: '+(rsn||'Admin'),timestamp:Date.now(),read:false});
    var modalBtns2=document.querySelectorAll('#manualWalletModal .btn-primary');modalBtns2.forEach(function(b){setLoading(b,false);});closeModal('manualWalletModal');showToast('✅ '+currSymbol+amt+' '+(act==='credit'?'credited':'debited'));
  }catch(e){var modalBtns3=document.querySelectorAll('#manualWalletModal .btn-primary');modalBtns3.forEach(function(b){setLoading(b,false);});showToast('Error: '+e.message,true);}}
...
function onEntryTypeChange(){
  ...
  // Auto-set prize type and labels based on entry type
  if(t==='paid'){
    h.className='info-box green';
    h.innerHTML='<i class="fas fa-info-circle"></i> <b>Paid Match</b> — Entry: 💠 Sky Diamond | Winner gets: <img src="green-diamond.png" style="width:14px;height:14px;vertical-align:middle;object-fit:contain;display:inline-block"> Green Diamond';
    if(entryFeeLabel) entryFeeLabel.textContent='💠 Entry Fee (Sky Diamond) *';
    if(prizeTypeEl) prizeTypeEl.value='greenDiamond';
    var lbl='<img src="green-diamond.png" style="width:14px;height:14px;vertical-align:middle;object-fit:contain;display:inline-block"> GD'; if(p1lbl)p1lbl.textContent='1st Prize ('+lbl+')'; if(p2lbl)p2lbl.textContent='2nd Prize ('+lbl+')'; if(p3lbl)p3lbl.textContent='3rd Prize ('+lbl+')';
  } else if(t==='coin'){
    h.className='info-box purple';
    h.innerHTML='<i class="fas fa-info-circle"></i> <b>Coin Match</b> — Entry: 🪙 Coins | Winner gets: 💠 Sky Diamond';
    if(entryFeeLabel) entryFeeLabel.textContent='🪙 Entry Fee (Coins) *';
    if(prizeTypeEl) prizeTypeEl.value='skyDiamond';
    var lbl2='💠 SD'; if(p1lbl)p1lbl.textContent='1st Prize ('+lbl2+')'; if(p2lbl)p2lbl.textContent='2nd Prize ('+lbl2+')'; if(p3lbl)p3lbl.textContent='3rd Prize ('+lbl2+')';
  } else {
    h.className='info-box yellow';
    h.innerHTML='<i class="fas fa-info-circle"></i> <b>Ad Match</b> — Entry: 📺 Watch Ads | Winner gets: 🪙 Coins';
    if(entryFeeLabel) entryFeeLabel.textContent='Entry Fee';
    if(prizeTypeEl) prizeTypeEl.value='coin';
    var lbl3='🪙 Coins'; if(p1lbl)p1lbl.textContent='1st Prize ('+lbl3+')'; if(p2lbl)p2lbl.textContent='2nd Prize ('+lbl3+')'; if(p3lbl)p3lbl.textContent='3rd Prize ('+lbl3+')';
  }
...
function editTournament(id){
  ...
  if(document.getElementById('tPrizeType')) document.getElementById('tPrizeType').value=d.prizeType||(d.entryType==='paid'?'greenDiamond':d.entryType==='coin'?'skyDiamond':'coin');
...
async function saveTournament(){
  ...
  var prizeType=(document.getElementById('tPrizeType')||{}).value||(et==='paid'?'greenDiamond':et==='coin'?'skyDiamond':'coin');
...
async function loadJoinedPlayers(){
  ...
  function _slotBadge(slot) {
    if (!slot) return '<span style="font-size:10px;color:rgba(255,255,255,.2)">—</span>';
    return '<span style="background:linear-gradient(135deg,rgba(0,212,255,.2),rgba(185,100,255,.2));border:1.5px solid rgba(0,212,255,.5);color:#00d4ff;border-radius:8px;padding:3px 10px;font-size:12px;font-weight:800;font-family:monospace;display:inline-block">' + slot + '</span>';
  }
...
async function publishResults(){
  ...
  if(!mid) return showToast('Select match',true);
  var t=currentTournamentData;
  
  // DOUBLE PAYMENT GUARD — check if already published
  var _existSnap=await rtdb.ref(DB_MATCHES+'/'+mid+'/status').once('value');
  var alreadyPublished=(_existSnap.val()==='resultPublished');
  ...
  /* DUPLICATE RANK CHECK before publish */
  var rankMap = {};
  var dupError = false;
  rows.forEach(function(row){
    if(row.dataset.isteam === '1') return; // skip team members for rank check
    var ri = row.querySelector('.rank-input');
    var rank = ri ? Number(ri.value) : 0;
    if(rank >= 1 && rank <= 3){
      if(rankMap[rank]){
        var nm1 = row.querySelector('td:nth-child(2)');
        showToast('❌ Duplicate Rank #'+rank+' — ek hi rank do players ko nahi de sakte! Pehle fix karo.', true);
    ...
    if(resultScreenshots.length>0){
      for(var si=0;si<resultScreenshots.length;si++){
        try{
          ...
          var sTask=await sRef.putString(resultScreenshots[si],'data_url');
    ...
    for(var i=0;i<rows.length;i++){
      ...
      if(alreadyPublished){
        ...
        if(tw>0){
          ...
          var _prizeType = t ? (t.prizeType || (t.entryType==='paid'?'greenDiamond':t.entryType==='coin'?'skyDiamond':'coin')) : 'coin';
          ...
          await rtdb.ref(DB_USERS+'/'+uid+'/notifications').push({title:'🏆 Match Result!',message:winMsg,timestamp:Date.now(),read:false,type:'result',uid:uid,matchId:mid});
        } else {
          await rtdb.ref(DB_USERS+'/'+uid+'/stats/winStreak').set(0);
          var noWinMsg='📋 '+(t?t.name:'')+' ka result publish ho gaya! Tumhara rank: '+(rank?'#'+rank:'Unranked')+', Kills: '+kills+'. Better luck next time! 💪';
          await rtdb.ref(DB_USERS+'/'+uid+'/notifications').push({title:'📋 Result Published — Dekho!',message:noWinMsg,timestamp:Date.now(),read:false,type:'result',uid:uid,matchId:mid});
        }
...
async function saveSettings(){try{
  ...
  await rtdb.ref('appSettings/payment').update({upiId:document.getElementById('settUpiId').value.trim(),payeeName:document.getElementById('settPayeeName').value.trim()||'Mini eSports',minWithdraw:Number(document.getElementById('settMinWithdraw').value)||50});
  if(rr) await rtdb.ref('appSettings/referralReward').set(Number(rr.value)||50);
  showToast('Settings saved!');
}catch(e){showToast('Error: '+e.message,true);}}
async function saveGameSettings(){try{await rtdb.ref('appConfig').update({referralReward:Number(document.getElementById('settReferralReward').value)||50});showToast('Saved!');}catch(e){showToast('Error: '+e.message,true);}}
...
window.showResultPublisher = window.showResultPublisher || function(mid, matchName) {
  showSection('results', document.querySelector('.nav-item[onclick*="results"]'));
  setTimeout(function() {
...
window._toggleRosterStatus = async function(key,idx){
  ...
  try{await rtdb.ref('joinRequests/'+key).update({rosterStatus:ns});window.loadRoster();showToast((ns==='kicked'?'🚫 Kicked: ':'✅ Restored: ')+r.ign);}
  catch(e){showToast('Error: '+e.message,true);}
};
;
...
window.showSkyDiamondRequests = function() {
  ...
  rtdb.ref('skyDiamondRequests').orderByChild('status').equalTo('pending').once('value', function(s) {
    if (!s.exists()) { body.innerHTML = '<div style="text-align:center;padding:30px;color:#888">No pending Sky Diamond requests</div>'; return; }
    var h = '<div style="padding:4px">';
    s.forEach(function(c) {
      ...
      h += '<div style="font-size:20px;font-weight:900;color:#00d4ff">💎 ' + r.amount + '</div></div>';
      h += '<div style="font-size:12px;color:#aaa;margin-bottom:10px">₹' + r.price + ' payment claimed</div>';
      h += '<div style="display:flex;gap:8px">';
      h += '<button onclick="approveSkyDiamond(\'' + id + '\',\'' + r.uid + '\',' + r.amount + ')" style="flex:1;padding:8px;border-radius:8px;background:rgba(0,212,255,.15);border:1px solid rgba(0,212,255,.3);color:#00d4ff;font-weight:700;cursor:pointer">✅ Approve</button>';
      h += '<button onclick="rejectSkyDiamond(\'' + id + '\')" style="flex:1;padding:8px;border-radius:8px;background:rgba(255,50,50,.1);border:1px solid rgba(255,50,50,.2);color:#ff5555;font-weight:700;cursor:pointer">❌ Reject</button>';
      h += '</div></div>';
    });
    h += '</div>';
    body.innerHTML = h;
