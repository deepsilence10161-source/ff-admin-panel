...
function getSupa() {
  /* ✅ FIX (live-testing): only return the client once it's actually
     authenticated (see window._supaAuthed, set by syncFirebaseToken).
     Callers here use getSupa() to gate admin-only reads/writes
     (fa66_checkAndCancelEmpty etc) — returning the pre-login anon
     client let them run against it and either fail or, worse, read
     wrong/empty data during the login window. */
  return (window._supa && window._supaAuthed) ? window._supa : null;
}
...
patchWhenReady('saveTournament', function () {
  ...
  window.saveTournament = async function () {
    ...
    if (proposedName && window.allTournaments) {
      ...
      /* ✅ BUG FIX (2026-09-06): window._editingTournamentId was never
         set anywhere in this codebase — always null — so "skip self
         when editing" never actually worked. Every edit-save of an
         existing match (name unchanged) matched itself and wrongly
         triggered the "Duplicate Match Name" confirm dialog. Read the
         real edit-target id from the #tournamentId hidden field
         instead, which admin-inline.js's saveTournament() already
         reliably populates for edits (empty for new matches). */
      var editingId = ((document.getElementById('tournamentId') || {}).value || '').trim() || null;
...
/* ════════════════════════════════════════════════════════════════════════
   BUG #12 (MINOR) — renderProfileRequests: trim fields
   BUG #1  (merged 2026-08 cleanup, was admin-fixes-v21.js) — XSS guard:
   strip any <script> tags injected via the proposed IGN/UID/phone fields
   after render, as defense-in-depth alongside admin-inline.js's own
   escapeHtml() (_eh) on those same fields.
   Problem: proposedIgn, proposedFfUid, proposedPhone not trimmed.
   Leading/trailing spaces can pass validation silently.
   Fix: Patch to apply .trim() to all proposed fields, then post-render
   strip any injected <script> tags.
   ════════════════════════════════════════════════════════════════════════ */
patchWhenReady('renderProfileRequests', function () {
  ...
  window.renderProfileRequests = function (snap) {
    ...
    /* Post-render: strip any <script> tags injected via XSS (defense in
       depth alongside admin-inline.js's own escapeHtml on these fields) */
  ...
  console.log('[admin-fixes] renderProfileRequests: .trim() + XSS guard applied (combined)');
});
...
patchWhenReady('approveProfileUpdate', function () {
  ...
  window.approveProfileUpdate = async function (rid) {
    ...
    /* ✅ FIX (live-testing, defensive): getDB() only checks existence,
       not bridge-readiness — use the bridge-aware rtdb directly instead,
       consistent with the rest of the codebase, so this can't silently
       read from raw Firebase for a Supabase-only path. */
...
/* ════════════════════════════════════════════════════════════════════════
   ...
   BUG #32 (merged 2026-08 cleanup, was admin-fixes-v21.js) — also skip
   when the admin isn't on a matches/dashboard-related section, for the
   same reason (avoid unnecessary reads/re-renders on unrelated screens).
   ════════════════════════════════════════════════════════════════════════ */
patchWhenReady('syncTournamentStatuses', function () {
  ...
  window.syncTournamentStatuses = function () {
    ...
    /* Skip execution when admin is on an unrelated section */
  ...
  console.log('[admin-fixes] syncTournamentStatuses: hidden-tab guard + section guard applied (combined)');
});
