...
/* ── LOAD TOURNAMENTS ── */
function loadSponsoredTournaments() {
  ...
  (window.rtdb||window.db).ref('sponsoredTournaments').orderByChild('createdAt').limitToLast(20).once('value', function(snap) {
    if (!snap.exists()) {
    ...
    var items = [];
    snap.forEach(function(c) { items.unshift({ id: c.key, d: c.val() }); });
...
window.approveSponsoredWd = function(reqId, uid, amount) {
  ...
  (window.rtdb||window.db).ref('walletRequests/' + reqId).update({ status: 'approved', approvedAt: Date.now() });
  // Notify user
  if (uid) {
