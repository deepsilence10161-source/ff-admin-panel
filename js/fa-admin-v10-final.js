...
window.showQuickCreate = function () {
  ...
  h += '<div style="display:flex;gap:6px"><input type="date" id="_qcDate" style="flex:1.2;padding:10px;border-radius:10px;border:1.5px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);color:#fff;font-size:13px;box-sizing:border-box"><input type="time" id="_qcTimeOnly" style="flex:1;padding:10px;border-radius:10px;border:1.5px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);color:#fff;font-size:13px;box-sizing:border-box"></div>';
  h += '<input type="hidden" id="_qcTime"></div>';
  ...
  /* Pre-set time */
  setTimeout(function () {
    /* ✅ BUG FIX (2026-08-29): populate the visible split date/time
       fields — _qcTime is now a hidden field only, combined from
       these two right before submission in _qcCreate(). */
    var dateEl = document.getElementById('_qcDate');
    var timeOnlyEl = document.getElementById('_qcTimeOnly');
    if (dateEl) {
      var now = new Date();
      now.setMinutes(0, 0, 0);
      now.setHours(now.getHours() + 1);
      ...
      if (timeOnlyEl) timeOnlyEl.value = pad(now.getHours()) + ':00';
    }
  }, 100);
};
...
window._qcCreate = async function () {
  console.log('[_qcCreate] BUILD 20260829a');
  ...
  /* ✅ BUG FIX (2026-08-29): "Match time select hi nahi ho raha" —
     ...
     native inputs, matching the fix applied everywhere else. Force
     blur() on both + wait two animation-frame ticks before reading. */
  var qcDateEl = document.getElementById('_qcDate');
  var qcTimeOnlyEl = document.getElementById('_qcTimeOnly');
  if (qcDateEl) qcDateEl.blur();
  if (qcTimeOnlyEl) qcTimeOnlyEl.blur();
  await new Promise(function(r){ requestAnimationFrame(function(){ requestAnimationFrame(r); }); });

  var qcDateVal = (document.getElementById('_qcDate') || {}).value || '';
  var qcTimeVal = (document.getElementById('_qcTimeOnly') || {}).value || '';
  var timeVal = (qcDateVal && qcTimeVal) ? (qcDateVal + 'T' + qcTimeVal) : '';
  ...
  if (Math.abs(matchTime - Date.now()) < 2*60*1000) {
    console.warn('[_qcCreate] matchTime suspiciously close to now:', new Date(matchTime), '— picker value may not have committed yet');
