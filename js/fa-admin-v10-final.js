...
window.showQuickCreate = function () {
  ...
  h += '<input type="datetime-local" id="_qcTime" onchange="window._onQcMatchTimeChange()" style="width:100%;padding:10px;border-radius:10px;border:1.5px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);color:#fff;font-size:13px;box-sizing:border-box"></div>';
...
window._qcCreate = async function () {
  console.log('[_qcCreate] BUILD 20260907a — double-read settle-check for datetime-local');
  var id = window._qcSelectedId;
  var tmpl = TEMPLATES.find(function (t) { return t.id === id; });
  if (!tmpl) { toast('Pehle template select karo!', true); return; }

  /* ✅ HARDENED (2026-09-07): same settle-check fix as the other 3
     match-creation paths — see admin-inline.js's saveTournament() for
     the full reasoning. */
  ...
  if (_qcTimeEl) {
    ...
    if (_qcRead1 !== _qcRead2) {
      console.warn('[_qcCreate] datetime-local value changed between reads — WebView commit race caught live:', _qcRead1, '→', _qcRead2);
    }
    var _qcSettled = _qcRead2 || _qcRead1;
    if (_qcSettled) window._qcCapturedMatchTime = _qcSettled;
  }
