...
window.showQuickCreate = function () {
  ...
  h += '<input type="datetime-local" id="_qcTime" onchange="window._onQcMatchTimeChange()" style="width:100%;padding:10px;border-radius:10px;border:1.5px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);color:#fff;font-size:13px;box-sizing:border-box">';
  h += '<div id="_qcTimePreview" style="margin-top:6px;font-size:12px;font-weight:700"></div></div>';
  ...
  window._qcCapturedMatchTime = ''; // fresh modal, fresh fields — reset the captured value

  /* Pre-set time to 1 hour from now */
  var _qcNow = new Date();
  _qcNow.setMinutes(0, 0, 0);
  _qcNow.setHours(_qcNow.getHours() + 1);
  window._qcCapturedMatchTime = _qcMsToLocalStr(_qcNow.getTime());
  setTimeout(function () {
    var el = document.getElementById('_qcTime');
    if (el) el.value = window._qcCapturedMatchTime;
    window._onQcMatchTimeChange();
  }, 100);
};
...
/* ══════════════════════════════════════════════════════════════
   ...
   same revert applied to the other 3 match-creation paths in this
   app. */
function _qcMsToLocalStr(ms) {
  var d = new Date(ms);
  return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0')+'T'+String(d.getHours()).padStart(2,'0')+':'+String(d.getMinutes()).padStart(2,'0');
}
window._onQcMatchTimeChange = function(){
  var el = document.getElementById('_qcTime');
  window._qcCapturedMatchTime = el ? el.value : '';
  var preview = document.getElementById('_qcTimePreview');
  if (preview && window._qcCapturedMatchTime) {
    var parts = window._qcCapturedMatchTime.split(/[-T:]/).map(Number);
...
window._qcCreate = async function () {
  console.log('[_qcCreate] BUILD 20260913a — trust captured value, no re-read at save time');
  ...
  /* ✅ ROOT CAUSE FOUND (2026-09-13) — see admin-inline.js's
     saveTournament() for the full story. Trust
     window._qcCapturedMatchTime directly; only fall back to a fresh
     DOM read if it's empty. */
  if (!window._qcCapturedMatchTime) {
    var _qcTimeEl = document.getElementById('_qcTime');
    if (_qcTimeEl) {
      _qcTimeEl.blur();
      await new Promise(function(r){ requestAnimationFrame(r); });
      var _qcRead1 = _qcTimeEl.value;
      await new Promise(function(r){ setTimeout(r, 120); });
      var _qcRead2 = _qcTimeEl.value;
      window._qcCapturedMatchTime = _qcRead2 || _qcRead1;
    }
  }
  var timeVal = window._qcCapturedMatchTime;
  ...
  /* ✅ BUG FIX (2026-08-28): third independent match-creation path
     found with the same fragile new Date(string) parsing already
     fixed twice elsewhere this session (admin-inline.js's
     saveTournament, features-admin.js's Quick Match Create) — this
     "Quick Create" template flow has its own separate timeVal parsing
     that had the identical issue. Same explicit-numeric-parts
     hardening applied. */
  var _qcParts = timeVal.split(/[-T:]/).map(Number);
  var matchTime = new Date(_qcParts[0], _qcParts[1]-1, _qcParts[2], _qcParts[3], _qcParts[4], 0, 0).getTime();
  ...
  if (Math.abs(matchTime - Date.now()) < 2*60*1000) {
    if (!confirm('⚠️ Match time abhi (' + new Date(matchTime).toLocaleString() + ') ke bahut kareeb hai — save hote hi match LIVE ho jayega. Sahi hai?')) return;
  }
...
function _initAllAlerts() {
  ...
  /* ✅ FIX (live-testing): getDB() only checks EXISTENCE of window.rtdb,
     which is true from page-load (raw Firebase), well before the
     Supabase bridge installs. The two permanent .on() listeners below
     would bind to raw Firebase if this ran too early — and since
     'matches' is a Supabase-only path (blocked in Firebase Rules by
     design), that means permanent permission_denied for the rest of
     the session, even after window.rtdb is later reassigned to the
     bridge. Wait for the bridge specifically before proceeding. */
  if (!db._isSupaBridge) { setTimeout(_initAllAlerts, 500); return; }
...
window.renderV10Attendance = function (matchId) {
  ...
  /* Real-time listener */
  /* ✅ FIX (live-testing, defensive): low-risk since this only runs on
     user button-click (bridge is almost certainly ready by then), but
     cheap insurance against the same permission_denied bug class as
     the other listeners in this file — bail with a clear message
     instead of binding to raw Firebase if the bridge somehow isn't
     ready yet. */
  if (!db._isSupaBridge) {
    var body = wrap.querySelector('[id*="ttBody"], tbody') || wrap;
    if (body) body.innerHTML = '<tr><td colspan="99" style="text-align:center;padding:20px;color:#888">Connecting… try again in a moment.</td></tr>';
    return;
  }
...
(function _initLiveNotify() {
  /* ✅ FIX (live-testing): two bugs here —
     1. The old retry checked `window.db` (Firestore, unrelated) instead
        of re-checking getDB(), so if getDB() was falsy once, this gave
        up silently forever instead of actually retrying.
     2. Even when getDB() WAS truthy, it only checked existence — true
        from page-load as raw Firebase, before the Supabase bridge
        installs. The permanent .on('child_changed') listener below
        would then bind to raw Firebase, and since 'matches' is
        Supabase-only (blocked in Firebase Rules by design), that means
        permanent permission_denied for the whole session. */
  var db = getDB();
  if (!db || !db._isSupaBridge) { setTimeout(_initLiveNotify, 500); return; }
...
/* ================================================================
   ✅ REMOVED (2026-08): 'INLINE ATTENDANCE SECTION' (Section 7) —
   was a second, page-level rendering of attendance (table layout,
   its own nav item + section-attendance HTML) built on the same
   underlying action functions (_v10MarkAbsent, _v10CopyIGNs,
   _v10StartMatch, _v10ToggleStatus) as renderV10Attendance's modal
   above. Having a dedicated sidebar nav item AND a button inside
   Joined Players that both opened attendance views was the exact
   duplication flagged — kept the modal (opened directly from Joined
   Players, no separate nav item needed), removed this inline page
   version, its sidebar nav item, and the section-attendance HTML
   block (index.html).
================================================================ */
