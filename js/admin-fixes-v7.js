...
(function () {
  ...
  window._saveDailyBonusConfig = function () {
    ...
    for (var i = 1; i <= 7; i++) {
      ...
      data['day' + i] = val;
      /* BUG #39 MITIGATION (2026-07-30): this editor previously saved any value up to the
         input's max=10000 with zero confirmation — a single fat-fingered digit (e.g. 60 typed
         as 600) goes live immediately for every user with no warning. This doesn't block large
         values (day 7 "legendary" reward may legitimately be big) but does make the admin
         explicitly confirm before saving anything that looks like a likely typo. */
      if (val > 100) suspicious.push('Day ' + i + ': ' + val + ' coins');
    }
  ...
  // ✅ BUG FIX (2026-08-23): "Daily bonus reward editor ko quick tools se
  // remove karde kyonki daily bonus reward editor feature setting
  // section me hai already". This floating action button (FAB) fallback
  // rendered on top of EVERY section — including Quick Tools — any time
  // #v7ToolsPanel injection into Settings hadn't succeeded yet (e.g. the
  // user opens the app straight into a different section and never
  // visits Settings that session, so the panel's target container is
  // never touched and injection keeps failing its `!settingsArea` check
  // silently). That's exactly why Daily Bonus Editor visibly appeared to
  // live in Quick Tools too — it wasn't a second copy of the feature,
  // it was this global overlay bleeding through on top of whatever
  // section happened to be open. The real, single copy already lives in
  // Settings (#v7ToolsPanel, injected below) — removing this duplicate
  // floating overlay entirely rather than trying to scope it away from
  // Quick Tools specifically, since it was never meant to be global.
  setTimeout(function () {
    if (window._v7PanelInjected) return;
    /* Retry the real Settings-panel injection instead of falling back to
       a floating global overlay — injectAdminButtons() is idempotent
       (guarded by settingsArea._v7Injected) so calling it again here is
       safe and just catches the case where Settings rendered late. */
    injectAdminButtons();
  }, 3000);
