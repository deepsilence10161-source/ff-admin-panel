/* ═══════════════════════════════════════════════════════════════════════
   ...
   ⚠️ MAINTENANCE NOTE (live-testing, 2026-08): this file is the FIRST of
   FOUR sequential patch files (v21 → v22-FINAL → v23-FINAL → v24-FINAL,
   in that load order per index.html). They use a shared patchWhenReady()
   pattern that WRAPS a function already on `window`, saving the current
   version as `_orig` and calling it internally. This means:
     1. If a later file (v22/23/24) patches the SAME function name again,
        it wraps whatever v21 (or v22/23) already produced — the chain is
        cumulative and ORDER-DEPENDENT. Do not reorder these four
        <script> tags in index.html without re-verifying every chained
        function still behaves correctly end-to-end.
     2. A fix made in a LATER file can silently make an EARLIER file's
        fix for the same function irrelevant (or, if done carelessly,
        undo it) — always grep all four files for a function name before
        assuming you've found its only definition. (This exact mistake
        broke a bridge-readiness fix for fa66_checkAndCancelEmpty during
        this testing round — it was fixed correctly in
        fa63-fa70-automation-bundle.js, then unknowingly re-wrapped with
        a weaker guard by admin-fixes-v24-FINAL.js, which loads later.)
     3. getDB() is defined separately in EACH of these four files (not
        shared) — they were unified during this session to all check
        window.rtdb._isSupaBridge rather than mere existence, but they
        remain four separate copies. If you change one, change all four.
   A full merge into one file was considered but deferred as too risky to
   do safely in one pass (49 patchWhenReady() calls across the four
   files, several of them chained); this note exists so the risk is
   understood rather than rediscovered from scratch.
...
/* ✅ FIX (live-testing — applies to EVERY call-site in this file that uses
   getDB(), ~13 of them): the old getDB() only checked whether SOME db
   handle existed (window.rtdb is truthy from page-load as raw Firebase,
   long before the Supabase bridge installs). Every caller here queries
   Supabase-only paths (matches, users, joinRequests, etc.) via this
   handle, so an early call would silently hit raw Firebase and throw
   permission_denied. Centralizing the bridge-readiness check here fixes
   every call-site in this file at once, consistent with the dedicated
   per-listener fixes already applied elsewhere in the codebase. Most
   callers here are click/action-triggered (by which point the bridge is
   normally long since ready) rather than self-starting, so this mainly
   closes a defensive gap rather than a currently-observed crash. */
...
/* ════════════════════════════════════════════════════════════════════════
   ...
   FIX: merged (2026-08 cleanup) into admin-fixes-v24-FINAL.js's
   renderProfileRequests wrapper (BUG #12) — both patched the same
   function via the same implicit patchWhenReady-timing mechanism, so
   they're now one explicit combined patch instead of two separate,
   order-dependent wraps.
   ════════════════════════════════════════════════════════════════════════ */

/* ════════════════════════════════════════════════════════════════════════
   BUG #6 — Race condition in profile approval (two admins same request)
   FIX: superseded (2026-08 cleanup) — admin_approve_profile (the RPC
   approveProfile calls, see admin-fixes-v25-SUPABASE.js) already has an
   in-memory JS lock (_apprLocks) AND the RPC itself does `SELECT ...
   FOR UPDATE` at the Postgres row level, which is a stronger guarantee
   than this file's old Firebase-status optimistic-lock wrapper. Removed
   to eliminate a redundant, implicit-timing-dependent wrap.
   ════════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════════
   BUG #15 — approveTeam missing bidirectional squad member link
   FIX: merged (2026-08 cleanup) into admin-fixes-v23-FINAL.js's
   approveTeam wrapper (FIX #6) — both patched the same function via
   the same implicit patchWhenReady-timing mechanism, so they're now
   one explicit combined patch instead of two separate, order-dependent
   wraps.
   ════════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════════
/* ════════════════════════════════════════════════════════════════════════
   BUG #32 (v21) — syncTournamentStatuses runs every 30s regardless of section
   FIX: merged (2026-08 cleanup) into admin-fixes-v24-FINAL.js's
   syncTournamentStatuses wrapper (BUG #20) — both patched the same
   function via the same implicit patchWhenReady-timing mechanism, so
   they're now one explicit combined patch instead of two separate,
   order-dependent wraps.
   ════════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════════
   ✅ REMOVED (2026-08): BUG #40 coin-request-badge polling — the entire
   Coin Requests feature was removed (coins are never purchasable in the
   User Panel, so this queue could never receive a request).
   ════════════════════════════════════════════════════════════════════════ */
...
/* ════════════════════════════════════════════════════════════════════════
   ...
   ✅ UPDATED (2026-08): renderLiveAttendance (old standalone sidebar
   section) was removed as a duplicate of renderV10Attendance (the modal
   opened from Joined Players) — see fa-admin-v10.js removal note. The
   scroll-preservation patch targeted only the old function, so it's
   removed too. Auto-refresh for Joined Players is kept below.
   ════════════════════════════════════════════════════════════════════════ */
(function _patchLiveAttendanceRefresh() {
  /* Bug#63: Auto-refresh joined players list every 30s when section is open */
  setInterval(function () {
    if (window.currentSection === 'joinedPlayers') {
      if (typeof window.loadJoinedPlayers === 'function') {
        window.loadJoinedPlayers(window._currentLiveMatchId || '');
      }
    }
  }, 30000);
  console.log('[v21] Bug#63 fix: Joined Players auto-refresh (30s) applied');
})();
  ...
  patchWhenReady('loadTournaments', function () {
    ...
    window.loadTournaments = async function (force) {
      ...
      if (!force && _cache && (now - _cacheTime) < _TTL) {
        ...
        /* ✅ FIX: window.renderTournaments is never defined anywhere in the
           codebase — this branch always silently no-op'd, leaving the
           visible table stale for up to 60s (and masking real updates if
           a later script wraps this function expecting a real render).
           Fall back to a full reload so the screen always reflects data. */
        if (typeof window.renderTournaments === 'function') { window.renderTournaments(_cache); return; }
        await _orig.apply(this, arguments);
...
/* ════════════════════════════════════════════════════════════════════════
/* ════════════════════════════════════════════════════════════════════════
   BUG #69 (v21) — showSection: track window.currentSection
   FIX: merged (2026-08 cleanup) into admin-fixes-v22-FINAL.js's
   showSection wrapper (also BUG #69) — both patched the same function
   via the same implicit patchWhenReady-timing mechanism, so they're
   now one explicit combined patch instead of two separate,
   order-dependent wraps.
   ════════════════════════════════════════════════════════════════════════ */
...
(function _userCacheRealtime() {
  /* ✅ FIX (live-testing): getDB() truthy only means SOME db handle exists
     (raw Firebase from page-load counts), not that the Supabase bridge is
     installed. These are PERMANENT .on() listeners on `users`, which
     firebase-rules.json only allows for 3 specific sub-fields (not the
     collection generally) — binding early means permanent
     permission_denied for the session. Wait for the bridge explicitly. */
...
(function _joinRequestsRealtime() {
  /* ✅ FIX (live-testing): same class as _userCacheRealtime above — wait
     for the actual Supabase bridge before binding permanent listeners on
     `joinRequests` (Supabase-only, not in the Firebase allowlist). */
  var iv = setInterval(function () {
    if (!(window.rtdb && window.rtdb._isSupaBridge)) return;
    var db = getDB();
...
/* ════════════════════════════════════════════════════════════════════════
   ...
   FIX: superseded (2026-08 cleanup) — see admin-fixes-v24-FINAL.js's
   BUG #6, which completely rewrites this function and removes the
   broken joinRequests/deviceMeta data source entirely (using only
   deviceJoins instead, which IS written by anti-cheat.js). v24's
   version never calls the original, so this file's guard-logic
   (checking a data source v24 doesn't even read anymore) was dead
   code — v24 loads after this file and fully replaces
   window.fa73_detectIPClusters rather than wrapping it.
   ════════════════════════════════════════════════════════════════════════ */

/* ════════════════════════════════════════════════════════════════════════
   BUG #111 — Prize calculation duplicated across 5+ functions
   FIX: superseded (2026-08 cleanup) — see admin-fixes-v22-FINAL.js's
   _unifyPrizeCalc, which fully replaces this with a version that
   includes currency resolution (fbPath/supaCol/symbol), which this
   version didn't have. This file's version was dead code (overwritten
   by v22 at load time, before anything ever called it).
   ════════════════════════════════════════════════════════════════════════ */
      ...
      if (!_isKnownDevice()) {
        ...
        if (_trustBtn) {
          _trustBtn.addEventListener('click', function() {
            ...
            var curEmail = (window.auth && window.auth.currentUser && window._adminEmail()) ?
              window._adminEmail().trim().toLowerCase() : null;
