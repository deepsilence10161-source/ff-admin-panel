...
(function () {
  ...
  window._goToAttendance = function (matchId) {
    ...
    /* ✅ FIX (2026-08): redirect to the richer Joined Players attendance
       modal (renderV10Attendance in fa-admin-v10-final.js) instead of the
       old standalone sidebar section, which had duplicate/lesser
       functionality and has been removed. */
    if (window.showSection) showSection('joinedPlayers', null);
    if (window.renderV10Attendance) window.renderV10Attendance(matchId);
  };
  ...
  function _initAlerts() {
    /* ✅ FIX (live-testing): window.rtdb existing ≠ Supabase bridge
       installed — this file loads before the bridge, so `db` was truthy
       (raw Firebase) almost immediately. Query `matches` (Supabase-only)
       only once the actual bridge is ready. */
    var db = window.rtdb || window.db;
    if (!db || !(window.rtdb && window.rtdb._isSupaBridge)) { setTimeout(_initAlerts, 500); return; }
  ...
  /* ✅ REMOVED (2026-08): dead standalone 'Live Attendance' sidebar
     section (renderLiveAttendance, _toggleAttendance, _markAllAbsent,
     _copyPresentIGNs + its own _statBox helper) — this was the older,
     less-capable implementation. Kept version: renderV10Attendance in
     fa-admin-v10-final.js (accessed via the 'Live Attendance' button
     inside Joined Players), which has everything this had (mark-absent,
     copy-IGNs, toggle-status) plus per-tab Auto Queue / Check-Ins views
     and a Start Match action. Having both caused the same feature to
     appear twice — once as its own sidebar nav item, once as a button
     inside Joined Players. The sidebar nav item + section-attendance
     HTML block were also removed (index.html). */
