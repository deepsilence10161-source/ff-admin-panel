...
(function () {
  ...
  /* ✅ FIX (live-testing): window.db is a bridge-aware SHIM that exists
     from script-load (admin-inline.js), independent of whether the
     Supabase bridge has actually installed — so this trigger condition
     was true almost immediately, well before window.rtdb._isSupaBridge.
     db.ref() on the un-bridged shim returns null, and .orderByChild()
     on null crashes; once the shim IS bridge-backed it queries `matches`
     (Supabase-only) which would 403 the same way as every other early
     listener in this audit if it ever raced ahead of the bridge. Wait
     for the bridge explicitly instead of just window.db/adminDb
     existing. */
  var _t = 0;
  var _iv = setInterval(function () {
    _t++;
    if (window.rtdb && window.rtdb._isSupaBridge) { clearInterval(_iv); setTimeout(startChecker, 3000); }
...
(function () {
  ...
  window.fa59_detectVIPUsers = function () {
    ...
    /* ✅ BUG FIX (2026-07-19): was selecting users.referral_count directly —
       that column has never existed on the users table (confirmed against
       COMPLETE_SCHEMA.sql; the live Postgres error logs showed this exact
       failure: "column users.referral_count does not exist... Perhaps you
       meant users.referral_code"). Per-user referral counts only exist as
       an aggregate, via the referral_leaderboard VIEW (GROUP BY referrer_id
       over the referrals table). Fetching both separately and merging
       client-side, since the view's INNER JOIN means users with zero
       referrals won't appear in it at all — those default to 0 below. */
    Promise.all([
      window._supa.from('users').select('id,sky_diamonds,total_matches,is_vip').eq('is_vip', false),
      window._supa.from('referral_leaderboard').select('referrer_id,referral_count')
    ]).then(function(results) {
      var r = results[0], refRes = results[1];
      var referralCounts = {};
      (refRes.data || []).forEach(function(row) { referralCounts[row.referrer_id] = row.referral_count; });
      var users = r.data || [];
      var vipFound = 0;
      var updates = [];
      users.forEach(function(u) {
        ...
        var referrals = Number(referralCounts[u.id]) || 0;
          ...
          window._supa.from('notifications').insert({ user_id:u.id, type:'vip', title:notif.title, body:notif.message, is_read:false }).then(null, function(){});
  ...
  // ✅ FIX (live-testing): wait for the actual Supabase bridge (not just the
  // window.db/adminDb shim) — this function's window.rtdb.ref('users/...')
  // calls would otherwise hit un-bridged Firebase for `users` (Supabase-only).
  // Run daily
