...
window.createSponsoredTournament = async function() {
  console.log('[createSponsoredTournament] BUILD 20260905c — reverted to native datetime-local');
  /* ✅ REVERTED (2026-09-05): read the native field directly — it's
     the source of truth again, no separate custom-picker step. */
  var _spTimeEl = document.getElementById('spTourMatchTime');
  if (_spTimeEl && _spTimeEl.value) _spCapturedMatchTime = _spTimeEl.value;
