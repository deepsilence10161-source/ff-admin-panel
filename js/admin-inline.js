...
function _adminUid() {
  try { return (window.auth && window.auth.currentUser && window._adminUid()) || 'unknown_admin'; }
  catch(e) { return 'unknown_admin'; }
}
function _adminEmail() {
  try { return (window.auth && window.auth.currentUser && window._adminEmail()) || 'unknown'; }
...
async function banUser(uid){
  ...
  try{
    ...
    if(window._supa){window._supa.rpc('set_user_ban_status',{p_uid:uid,p_banned:true,p_reason:r}).catch(function(e){console.warn('[BUG#40] banUser Supabase:',e.message);});}
    /* Refresh cache */
...
async function deleteUser(uid){
  ...
  try{
    ...
    if(window._supa){
      window._supa.rpc('set_user_ban_status',{p_uid:uid,p_banned:true,p_reason:'Account deleted'}).catch(function(){});
          ...
          if(window._supa && tw > 0){
            ...
            if(rank===1){
              window._supa.from('users').select('win_streak').eq('id',uid).single()
                .then(function(r){ var cur=(r.data&&r.data.win_streak)||0; window._supa.from('users').update({win_streak:cur+1}).eq('id',uid).then(null, function(){}); }).catch(function(){});
            } else {
...
async function approveCoinRequest(reqId, userId, coins) {
  ...
  try {
    ...
    await rtdb.ref('coinRequests/' + reqId).update({ status: 'approved', processedAt: Date.now(), processedBy: _adminUid() });
