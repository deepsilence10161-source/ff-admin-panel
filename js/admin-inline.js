...
async function saveTournament(){
  console.log('[saveTournament] BUILD 20260905c — reverted to native datetime-local (v20 style)');
  var saveBtn=document.getElementById('saveTournamentBtn');
  setLoading(saveBtn,true);

  /* ✅ REVERTED (2026-09-05): back to reading the native
     datetime-local input directly — it IS the source of truth now,
     there's no separate custom-picker variable to sync it from.
     _onTMatchTimeChange() (fired on the field's own 'change' event)
     keeps _capturedMatchTime in sync as a convenience/fallback only;
     the field's live .value is what's actually used here. */
  var tMatchTimeEl = document.getElementById('tMatchTime');
  if (tMatchTimeEl && tMatchTimeEl.value) _capturedMatchTime = tMatchTimeEl.value;
