...
window.injectGrowthNavItems = function() {
  ...
  `;
  /* ✅ REMOVED (2026-09-09), per Junaid's request to clean up the
     settings navigation mess: this group's own "App Settings" nav
     item used to point to showSection('appSettings',...) — an id
     that's now merged into the main sidebar's "Settings" item
     (id="section-settings", see index.html's comment on that
     section for the full history). Two separate nav items opening
     the same merged content is exactly the kind of duplication
     Junaid asked to clean up — removed this one; the main sidebar's
     "Settings" (which now correctly calls loadAppSettings() itself
     too, see index.html) is the single entry point. */
