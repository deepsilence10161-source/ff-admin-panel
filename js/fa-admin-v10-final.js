...
window._qcCreate = async function () {
  console.log('[_qcCreate] BUILD 20260905c — reverted to native datetime-local');
  var id = window._qcSelectedId;
  var tmpl = TEMPLATES.find(function (t) { return t.id === id; });
  if (!tmpl) { toast('Pehle template select karo!', true); return; }

  /* ✅ REVERTED (2026-09-05): read the native field directly — it's
     the source of truth again. */
  var _qcTimeEl = document.getElementById('_qcTime');
  if (_qcTimeEl && _qcTimeEl.value) window._qcCapturedMatchTime = _qcTimeEl.value;
