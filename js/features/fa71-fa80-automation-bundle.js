...
(function () {
  ...
  window._fa71Schedule = function () {
    ...
    db.ref('scheduledBroadcasts').push({
      ...
      targetType: target || 'all', sendAt: scheduleAt,
  ...
  function checkScheduledBroadcasts() {
    ...
    db.ref('scheduledBroadcasts').orderByChild('sent').equalTo(false).once('value', function (snap) {
      ...
      snap.forEach(function (c) {
        ...
        if (d.sendAt <= now) {
          ...
          // Mark as sent
          db.ref('scheduledBroadcasts/' + c.key).update({ sent: true, sentAt: now });
        }
      });
    });
  }

  // ✅ FIX (live-testing): window.db/adminDb existing ≠ Supabase bridge
  // installed — wait for _isSupaBridge before this hits `scheduledBroadcasts`
  // and `users` (both Supabase-only).
  // Check every 5 min
...
(function () {
  ...
  // ✅ FIX (live-testing): wait for actual Supabase bridge, not just the
  // window.db shim, before this hits `leaderboard`/`users` (Supabase-only).
  // Run weekly
