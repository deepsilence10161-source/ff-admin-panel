...
/* ── Auto-init on load ──
   ✅ FIX (live-testing): no bridge-readiness check at all here — this ran
   unconditionally on DOMContentLoaded/1500ms and bound a PERMANENT
   .on('value') listener on `adminAlerts`, which is NOT in
   firebase-rules.json's allowlist (only 'adminActions' is — different
   path, easy to confuse). If window.rtdb wasn't the Supabase bridge yet
   when this fired, the listener binds to raw Firebase permanently for
   the session and throws permission_denied on every update. Wait for the
   bridge. */
function _waitForBridgeThenFraudAlerts(waited) {
  waited = waited || 0;
  if (window.rtdb && window.rtdb._isSupaBridge) {
    window.initFraudAlerts();
    return;
  }
  if (waited >= 20000) {
    console.warn('[fa25-fraud-dashboard] Supabase bridge never became ready — fraud alerts listener not started.');
    return;
  }
  setTimeout(function(){ _waitForBridgeThenFraudAlerts(waited + 300); }, 300);
}
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function(){ setTimeout(_waitForBridgeThenFraudAlerts, 1500); });
} else {
  setTimeout(_waitForBridgeThenFraudAlerts, 1500);
}
