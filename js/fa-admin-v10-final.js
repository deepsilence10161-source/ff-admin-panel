...
(function () {
  var db = getDB();
  if (!db) { setTimeout(arguments.callee, 3000); return; }
...
window.endCurrentSeason = function() {
  ...
  db_.ref('appSettings/currentSeason').once('value', function(snap) {
    ...
    // Get top 100 players by rank points
    db_.ref('users').orderByChild('stats/rankPoints').limitToLast(100).once('value', function(uSnap) {
      var players = [];
      uSnap.forEach(function(c) { players.push({ uid: c.key, u: c.val() }); });
      players.sort(function(a,b) {
        return (b.u.stats&&b.u.stats.rankPoints||0) - (a.u.stats&&a.u.stats.rankPoints||0);
      });

      // Award badges
      players.forEach(function(p, i) {
        var pos = i + 1;
        var badge = pos === 1 ? '🌟 Season Champion' :
                    pos <= 5  ? '👑 Season Legend' :
                    pos <= 20 ? '💎 Season Diamond' : '🥇 Season Gold';
        var reward = pos === 1 ? '+500🪙' : pos <= 5 ? '+200🪙' : pos <= 20 ? '+100🪙' : '+50🪙';
        var coins = pos === 1 ? 500 : pos <= 5 ? 200 : pos <= 20 ? 100 : 50;

        db_.ref('users/' + p.uid + '/coins').transaction(function(v) { return (v||0) + coins; });
        db_.ref('users/' + p.uid + '/seasonHistory').push({
          seasonId: seasonId, seasonName: season.name || 'Season',
          position: pos, badge: badge, reward: reward, emoji: badge.split(' ')[0],
          endDate: Date.now()
        });
        db_.ref('users/' + p.uid + '/notifications').push({
          type: 'season_end', title: '🏆 Season Ended!',
          message: season.name + ' mein tumhara rank: #' + pos + '! ' + badge + ' mila! ' + reward + ' bonus!',
      ...
      if (window.showToast) showToast('✅ Season ended! ' + players.length + ' players rewarded.', false);
    });
  });
};
