...
(function () {
  ...
  function _cleanNode(node) {
    ...
    /* ✅ FIX (live-testing, 2026-08 — THE confirmed root cause of
       Approve/Reject/every-inline-onclick-button silently doing
       nothing across this entire codebase, found only after
       exhausting every other explanation): this observer fires on
       EVERY new DOM node anywhere under document.body, including
       every table row this app renders via innerHTML +=
       '<button onclick="approveProfile(...)">' — which is the
       standard, deliberate pattern used for interactivity throughout
       this codebase (hundreds of onclick attributes across dozens of
       files). The unconditional `/^on\w+/i.test(attr.name)` below
       stripped ALL of them within milliseconds of insertion, long
       before a user could ever click — the button would render
       correctly, look identical, but its onclick was already gone by
       the time anyone tapped it. This explains every symptom seen
       during testing: no console error (nothing crashed — the handler
       just didn't exist), no toast, no alert (none of the JS inside
       the handler ever ran), and even a bare inline
       `this.style.background='lime'` test not firing (confirming the
       attribute itself, not just its JS content, was gone).
       This was intended as XSS protection against attacker-injected
       content (e.g. malicious chat messages, user-submitted profile
       text rendered unsafely), not against this app's own
       hardcoded, developer-authored onclick handlers — those aren't
       untrusted input. Only strip on* attributes whose VALUE looks
       like it was injected from external/untrusted data (contains
       things a legitimate hardcoded handler in this codebase would
       never contain: cross-origin fetch/XHR to a non-Supabase host,
       document.cookie access, eval, or base64 data: URIs) rather than
       stripping every on* attribute unconditionally. */
    var _SUSPICIOUS_ONHANDLER = /document\.cookie|eval\s*\(|atob\s*\(|data:text\/html|fetch\s*\(\s*["'`](?!.*supabase\.co)https?:\/\//i;
    var attrs = Array.prototype.slice.call(node.attributes || []);
    attrs.forEach(function (attr) {
      if (/^on\w+/i.test(attr.name) && _SUSPICIOUS_ONHANDLER.test(attr.value || '')) {
