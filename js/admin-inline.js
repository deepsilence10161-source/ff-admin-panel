...
/* ✅ FIX (2026-08-17, CRITICAL): This used to build allWalletRequests purely
   from Firebase walletRequests — but quick-deposit.js (User Panel) never
   actually wrote UTR/screenshot into that Firebase node at all (see
   quick-deposit.js fix); it only reached Supabase sd_requests, and even
   there the UTR/screenshot were being silently dropped before this fix.
   Now that quick-deposit.js correctly saves screenshot_url/upi_ref to
   Supabase, this table must read sky-diamond-purchase rows from Supabase
   (the real source of truth) so UTR/UPI and the screenshot actually show
   up. Withdrawal-type rows (sponsored prize payouts) are untouched here —
   they live in a separate table (wallet_transactions) and weren't part of
   the reported bug — so they still come from Firebase as before. */
function setupWalletListener(){
  rtdb.ref(DB_WALLET).on('value',function(s){
    var fbRequests={};
    s.forEach(function(c){
      var w=c.val();
      /* Only keep withdrawal-type rows from Firebase — sky diamond
         purchase rows now come from Supabase (below) since that's where
         UTR/screenshot actually live. */
      if(normalizeWalletType(w.type)==='withdraw') fbRequests[c.key]=w;
    });
    _refreshSdRequestsIntoWallet(fbRequests);
  });
}
async function _refreshSdRequestsIntoWallet(fbRequests){
  allWalletRequests = fbRequests || {};
  try{
    if(window._supa){
      var res = await window._supa.from('sd_requests')
      ...
      if(!res.error && res.data){
        res.data.forEach(function(r){
          allWalletRequests[r.id]={
            uid:r.user_id, userName:r.ign, ffUid:'', type:'add',
            diamonds:r.sd_amount, amount:r.amount_inr,
            screenshotUrl:r.screenshot_url, utrNumber:r.upi_ref,
  ...
  }catch(e){ console.error('[Wallet] Supabase sd_requests fetch failed:',e.message); }
...
async function approveAddMoney(rid, evt){
  /* ✅ FIX (live-testing — CRITICAL): bare `event.target...` with NO
     `event &&` guard and NO try/catch around it yet — if `event` was
     ever genuinely undefined (not just falsy) for this call path, this
     line alone throws ReferenceError: event is not defined and kills
     the function before the try block even starts, before ANY network
  ...
  try{
    ...
    await rtdb.ref(DB_WALLET+'/'+rid).update({status:'approved',processedAt:Date.now(),processedBy:_adminUid()});
    ...
    /* Log activity */
    await rtdb.ref('activityLogs').push({type:'wallet_approved',uid:uid,amount:amt,walletType:'deposit',admin:_adminUid(),timestamp:Date.now()});
...
async function confirmWithdrawal(){
  ...
  try{
    ...
    /* Log activity */
    try{await rtdb.ref('activityLogs').push({type:'withdrawal_approved',uid:uid,amount:amt,admin:_adminUid(),timestamp:Date.now()});}catch(le){}
    try{await rtdb.ref('walletAuditLog').push({uid:uid,action:'WITHDRAWAL_APPROVED',amount:amt,note:'UPI: '+upiId+' | TxnID: '+txnId+' | Admin: '+_adminEmail(),timestamp:Date.now(),adminUid:_adminUid()});}catch(le){}
    setLoading(btn,false);
...
async function submitReject(){
  ...
  try{
    if(type==='profile'){
      ...
      /* Update wallet request status */
      await rtdb.ref(DB_WALLET+'/'+requestId).update({status:'rejected',rejectionReason:rsn,processedAt:Date.now(),processedBy:_adminUid()});
...
async function approveTeam(rid){
  try{
    ...
    await rtdb.ref(DB_TEAM+'/'+rid).update({status:'approved',processedAt:Date.now(),processedBy:_adminUid()});
