...
function getDB(){
  /* ✅ FIX (2026-08 — 2nd root cause of blank v17 sections): window.db is
     the Firestore client (unrelated, unused elsewhere in this codebase —
     see supabase-rtdb-bridge.js's explicit "DO NOT override it" note),
     not the RTDB-style bridge these functions actually call .ref()/
     .once() on. Every load function below would crash with "db.ref is
     not a function" since Firestore has no .ref() method. window.rtdb is
     the correct handle. These sections are only opened on-demand (nav
     click), so the bridge is almost always ready by then, but bail
     cleanly instead of silently hitting raw pre-bridge Firebase (which
     would permission_denied on these Supabase-only paths) if clicked
     within the first moment of page load. */
  if (window.rtdb && window.rtdb._isSupaBridge) return window.rtdb;
  return null;
}
  ...
  if(main){
    ['bracketAdmin','clanWarAdmin','cityChampAdmin','mentorAdmin','cleanBadgeAdmin'].forEach(function(id){
      if(!document.getElementById('section-'+id)){
        ...
        sec.className='section'; sec.id='section-'+id;
        /* ✅ FIX (2026-08 — root cause of blank Brackets/Clan Wars/City
           Champ/Mentors/Clean Badges screens): removed inline
           style.display='none'. The .section CSS class already handles
           display:none by default and .section.active sets
           display:block — but inline styles have higher specificity
           than class rules, so this permanently overrode the .active
           class added by showSection(), keeping every one of these 5
...
window.loadBracketAdmin = function(){
  ...
  .then(function(res){ var list=(res.data||[]);
    ...
    list.forEach(function(b){
      ...
      h+='<div style="font-size:12px;color:var(--txt2)">'+(b.format||'Single Elimination')+' · '+(b.team_count||0)+' teams</div></div>';
      h+='<div style="display:flex;gap:8px;align-items:center">';
      h+='<span style="font-size:12px;font-weight:700;color:'+statusColor+'">'+(b.status||'upcoming').toUpperCase()+'</span>';
      h+='<button onclick="window.editBracket(\''+b.id+'\')" style="padding:6px 10px;border-radius:8px;border:1px solid var(--border);background:transparent;cursor:pointer;font-size:11px">Edit</button>';
      h+='<button onclick="window.deleteBracket(\''+b.id+'\')" style="padding:6px 10px;border-radius:8px;border:1px solid rgba(255,107,107,.3);background:transparent;color:#ff6b6b;cursor:pointer;font-size:11px">Delete</button>';
      h+='</div></div>';
...
window.showCreateBracketForm = function(){
  var h='';
  h+='<div style="display:flex;flex-direction:column;gap:10px">';
...
window._saveBracket=function(){
  ...
  .then(function(){ toast('✅ Bracket created!'); if(window.closeModal)closeModal(); if(window.loadBracketAdmin)loadBracketAdmin(); }, function(e){ toast('Error: '+(e.message||'Try again'),true); });
};
...
window.editBracket=function(id){
  var s=_supa(); if(!s){ toast('DB error',true); return; }
  s.from('tournament_brackets').select('id,name,status,champion,rounds').eq('id',id).single()
  .then(function(res){
    var b=res.data; if(!b){ toast('Bracket not found',true); return; }
    var rounds=[]; try{ rounds=JSON.parse(b.rounds||'[]'); }catch(e){ rounds=[]; }
    /* Build round editor */
  ...
  }, function(e){ toast('Error: '+(e.message||'Try again'),true); });
};

window._updateBracket=function(id){
  var s=_supa(); if(!s){ toast('DB error',true); return; }
  s.from('tournament_brackets').select('rounds').eq('id',id).single()
  .then(function(res){
    var b=res.data; if(!b){ toast('Bracket not found',true); return; }
    var rounds=[]; try{ rounds=JSON.parse(b.rounds||'[]'); }catch(e){ rounds=[]; }
    var status=(document.getElementById('brkEditStatus')||{}).value;
    var champion=((document.getElementById('brkEditChampion')||{}).value||'').trim();
    rounds=rounds.map(function(round,ri){
    ...
    var upd={rounds:JSON.stringify(rounds),updated_at:new Date().toISOString()};
    if(status) upd.status=status;
    if(champion) upd.champion=champion;
    return s.from('tournament_brackets').update(upd).eq('id',id);
  })
  .then(function(res){
...
window.deleteBracket=function(id){
  ...
  var s=_supa(); if(!s){ toast('DB error',true); return; }
  s.from('tournament_brackets').delete().eq('id',id)
  .then(function(res){
...
window.loadCityChampAdmin=function(){
  ...
  db.ref('cityChampionship/'+month+'/cities').orderByChild('score').limitToLast(20).once('value',function(s){
    /* ✅ FIX: snap.key is always the row's own UUID 'id' per this bridge's
...
window.loadMentorAdmin=function(){
  ...
  c.innerHTML='<div class="loading-state"><i class="fas fa-spinner fa-spin"></i></div>';
  var db=getDB(); if(!db){ c.innerHTML='<p style="color:#ff6b6b">DB error</p>'; return; }
  /* ✅ FIX: mentor_profiles column is 'is_available', not 'active'.
     Query 'isAvailable' so the bridge's camelCase->snake_case conversion
...
window._cbSearch=function(q){
  ...
  _cbTimer=setTimeout(function(){
    ...
    .then(function(res){
      ...
      users.forEach(function(u){
        /* ✅ FIX: bridge maps flat fields 'cleanMatches'->clean_matches and
           'hasCleanBadge'->has_clean_badge on the users table. The old code
           read a nested 'cleanRecord' object that the bridge never mapped,
           so this always silently returned empty (0/30, badge never shown). */
        Promise.all([
          new Promise(function(res){ db.ref('users/'+u.id+'/cleanMatches').once('value', function(s){ res(s.val()); }); }),
          new Promise(function(res){ db.ref('users/'+u.id+'/hasCleanBadge').once('value', function(s){ res(s.val()); }); })
        ]).then(function(vals){
          var rec = { cleanMatches: vals[0] || 0, hasBadge: !!vals[1] };
    ...
    }, function(){ r.innerHTML='<div style="color:#ff6b6b;text-align:center">Error</div>'; });
  },300);
};
...
window._revokeCleanBadge=function(uid){
  ...
  var db=getDB(); if(!db) return;
  db.ref('users/'+uid+'/hasCleanBadge').set(false,function(){
    db.ref('users/'+uid+'/cleanMatches').set(0);
