...
document.addEventListener('DOMContentLoaded', function() {
  /* ✅ FIX (live-testing): these two permanent .on('value') listeners
     are on Supabase-mapped paths (cheatReports/refundRequests) — if
     they ran before the Supabase bridge installed (rt() being truthy
     only means raw Firebase RTDB exists, which is true from page-load,
     well before the bridge is ready), they'd bind to raw Firebase and
     stay bound for the whole session, permanently throwing
     permission_denied. Wait for the bridge specifically, same fix
     pattern as elsewhere this session. */
  (function _waitForBridge(waited) {
    ...
    if (waited >= 15000) {
      console.warn('[fa27] Supabase bridge never became ready — cheat report / refund badge listeners not started.');
      return;
    }
