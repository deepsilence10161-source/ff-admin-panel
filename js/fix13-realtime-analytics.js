...
(function() {
  ...
  function registerPresence() {
    ...
    var uid = (window.currentAdminUid || (window.auth && window.auth.currentUser && window._adminUid()) || 'admin');
  ...
  function listenPresence() {
    ...
    function pollSupabaseActive() {
      ...
      /* ✅ FIX (live-testing): this whole module self-starts on
         DOMContentLoaded, independent of admin login — same race class
         as admin-supabase-sync.js's watchers (fixed earlier this audit).
         window._supa exists as an ANON client from page-load; only
         window._supaAuthed (set after syncFirebaseToken) means it's
         actually authenticated. The 'users' table's active-count read
         doesn't strictly need admin RLS, but waiting avoids noisy
         pre-auth 401s/empty-result flicker on the dashboard badge. */
      ...
      /* ✅ FIX (BUG L-6 class): count:'exact'+head:true is unreliable
         under headless-Chromium test conditions (100% ERR_ABORTED in
         testing elsewhere in this codebase, confirmed via direct SQL
         that the equivalent filter/count works fine — transport-layer
         quirk, not a data bug). Switched to a capped row select + length;
         this only runs every 30s, so the extra payload is a non-issue. */
      supa.from('users')
        .select('id')
        .gte('last_seen', tenMinAgo)
        .limit(5000)
        .then(function(r) {
          var supaCount = (r && r.data) ? r.data.length : 0;
          /* Merge with Firebase presence (admin sessions) */
          var total = supaCount + Object.keys(_presenceData).length;
          _recordBucket(total);
          updatePanel(total);
        }, function() { updatePanel(Object.keys(_presenceData).length); });
    }
    pollSupabaseActive();
