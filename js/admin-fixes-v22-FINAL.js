...
/* ✅ FIX (live-testing): same centralized bridge-readiness fix as
   ...
   Applies to every call-site in this file. */
...
/* ═══════════════════════════════════════════════════════════════════════════
   BUG FIX #2 — approveProfile "processing" lock never resets on error
   Superseded (2026-08 cleanup): this wrapper existed specifically to
   reset the Firebase status='processing' lock that admin-fixes-v21.js
   used to set — that lock has been removed as redundant (see the
   corresponding cleanup note in admin-fixes-v21.js), so there is no
   longer a 'processing' status to reset. The current approveProfile
   (admin-fixes-v25-SUPABASE.js) already shows errors to the admin and
   clears its own lock in a `finally` block regardless of outcome, so
   retry already works correctly without this wrapper.
   ═══════════════════════════════════════════════════════════════════════════ */
...
(function _fixSectionListenerLeaks() {
  ...
  patchWhenReady('showSection', function () {
    ...
    window.showSection = function (section, el, skipHistory) {
      window.currentSection = section; /* merged from v21 Bug#69 — track for other code that reads it */
      ...
      if (SECTION_LISTENERS[section]) {
        /* ✅ FIX (live-testing, 2026-08): same bridge-readiness bug class
           fixed elsewhere this session — if this ran before the
           Supabase bridge installed (window.rtdb exists as raw
           Firebase from page-load, well before the bridge is ready),
           these listeners would permanently bind to raw Firebase for
           these Supabase-only paths, throwing permission_denied for
           the rest of the session even after the bridge installs
           moments later. Guard against that. */
        if (window.rtdb && window.rtdb._isSupaBridge) {
          ...
          console.warn('[admin-fixes] showSection: Supabase bridge not ready yet — deferring listener setup for section:', section);
        }
      }
...
/* ═══════════════════════════════════════════════════════════════════════════
   BUG #73 — runFraudCheck loads the ENTIRE deviceJoins node at once
   FIX: superseded (2026-08 cleanup) — admin-fixes-v24-FINAL.js does a
   complete replace of runFraudCheck (its own comment says so) that
   always uses .limitToLast(1000), no confirm-dialog step needed. This
   file's confirm-dialog-based version, and the db.ref monkey-patch
   that supported it, were both dead code — v24 loads after this file
   and fully replaces window.runFraudCheck rather than wrapping it, so
   this version's logic (and its 'pagination active' flag) never ran.
   ═══════════════════════════════════════════════════════════════════════════ */
...
(function _tournamentAutoRefresh() {
  ...
  function startPoll() {
    ...
    /* ✅ SPEED FIX (2026-08-24): was 60s poll-ONLY — matches/join_requests
       are now in the supabase_realtime publication (this session's DB
       delta), so subscribe live and let this interval become a rare
       safety net instead of the only way tournament data ever updates.
       Force-refresh interval widened accordingly since realtime is now
    ...
    }, 60000); /* safety-net only now — realtime below is primary */
    console.log('[v22] Bug#85 Fix: Tournament auto-refresh poll started (60s safety-net)');

    /* Live: any matches or join_requests change reloads immediately,
       regardless of which admin section is currently open, so switching
       to Tournaments always shows current data with zero wait. */
    try {
      window._supa.channel('v22_tournaments_live')
  ...
  /* ✅ FIX (live-testing): loadTournaments() queries `matches` and
     `joinRequests` (both Supabase-only) — starting this 5s after script
     load, unconditionally, could fire before the Supabase bridge installs
     and throw permission_denied on raw Firebase. Wait for the bridge. */
  (function _waitThenStartPoll(waited) {
    waited = waited || 0;
    if (window.rtdb && window.rtdb._isSupaBridge) { startPoll(); return; }
    if (waited >= 20000) { setTimeout(startPoll, 1000); return; } /* give up waiting, but still start eventually */
