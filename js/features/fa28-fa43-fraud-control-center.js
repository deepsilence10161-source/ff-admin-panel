...
window.showFraudScoreDashboard = function() {
  ...
  db.ref('users').orderByChild('penaltyPoints').startAt(1).limitToLast(20).once('value', function(s) {
    ...
    if (s.exists()) s.forEach(function(c) {
      ...
      /* ✅ FIX (live-testing — confirmed root cause of admin_set_fraud_score
         anon-role permission-denied errors from Supabase logs): only
         checked window._supa exists (anon client from page-load), not
         window._supaAuthed (set after syncFirebaseToken authenticates
         it). This dashboard is opened via button click, so it could
         fire against the anon client if clicked before auth completes. */
      if (window._supa && window._supaAuthed) {
        window._supa.rpc('admin_set_fraud_score', { p_uid: c.key, p_score: score.total })
          .then(function(res){ if (res.error) console.warn('[fa28] fraud_score persist fail', c.key, res.error.message); })
...
document.addEventListener('DOMContentLoaded', function() {
  /* ✅ FIX (live-testing): initFraudAlertBadge binds a PERMANENT
     .on('child_added') listener on adminAlerts, which is a
     Supabase-mapped path — if this ran before the Supabase bridge
     installed (rt() being truthy only means raw Firebase RTDB exists,
     which is true from page-load, well before the bridge is ready),
     it would bind to raw Firebase and stay bound for the whole
     session, permanently throwing permission_denied. Wait for the
     bridge specifically, same fix pattern as elsewhere this session. */
  (function _waitForBridge(waited) {
    waited = waited || 0;
    ...
    if (waited >= 15000) {
      console.warn('[fa28-43] Supabase bridge never became ready — fraud alert badge listener not started.');
      return;
    }
    setTimeout(function(){ _waitForBridge(waited + 300); }, 300);
  })();
});
