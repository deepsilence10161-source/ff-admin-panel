...
window.createSponsoredTournament = async function() {
  console.log('[createSponsoredTournament] BUILD 20260907a — double-read settle-check for datetime-local');
  /* ✅ HARDENED (2026-09-07): same settle-check fix as
     admin-inline.js's saveTournament(), applied here too — live DB
     data confirmed the time-mismatch bug is an INTERMITTENT Android
     WebView commit race in the native datetime-local input itself,
     not a deterministic logic bug, so every match-creation path using
     this input type needs the same double-read hardening. */
  ...
  if (_spTimeEl) {
    ...
    if (_spRead1 !== _spRead2) {
      console.warn('[createSponsoredTournament] datetime-local value changed between reads — WebView commit race caught live:', _spRead1, '→', _spRead2);
    }
    var _spSettled = _spRead2 || _spRead1;
    if (_spSettled) _spCapturedMatchTime = _spSettled;
  }
